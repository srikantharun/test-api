// Description: conv2d kernel for 7x7 convolution
  ...
  for (int32_t c = 0; c < num_columns; c += block_size_c) {
    ...
    for (int32_t r = 0; r < num_rows; r += block_size_o) {
      ...
      for (int32_t idx = 1; idx < F - 1; idx += 2) {
        ...
        // start calculating their contributions on the two output rows (v0, v2)
        ...
        i_       = i + c_ + idx;
        sld_elem = *i_;
        asm volatile("vmacc.vx v0, %0, v29" ::"r"(t6));
        f_ = f + idx + 1;
        t0 = *f_;
        f_ += lwf;
        asm volatile("vmacc.vx v2, %0, v29" ::"r"(t5));

        asm volatile("vslide1down.vx v4, v20, %0" ::"r"(sld_elem));
        i_       += lwi;
        sld_elem = *i_;
        asm volatile("vmacc.vx v2, %0, v31" ::"r"(t6));
...
void conv2d_7x7_i8(int8_t *o, int8_t *i, int8_t *f, int32_t num_rows, int32_t num_columns) {
  ...
  // Set the vector configuration
  asm volatile("vsetvli %0, %1, e8, m1, ta, ma" : "=r"(block_size_c) : "r"(num_columns));
  ...
  for (int32_t c = 0; c < num_columns; c += block_size_c) {
    ...
    i__ += lwi;
    asm volatile("vle8.v v6,  (%0)" ::"r"(i__));
    i__ += lwi;
