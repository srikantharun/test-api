...
#error "Please define a way to initialize a memory block."
    ...
    {
        // TODO(mwipfli): Add this back when adding timing functions.
        // ee_printf(
    ...
    if (total_errors == 0)
    {
        ...
        if (known_id == 3)
        {
            ...
            if (time_in_secs(total_time) > 0)
                performance = default_num_contexts * results[0].iterations
                    / time_in_secs(total_time);
    ...
    return (total_errors == 0) ? TEST_SUCCESS : TEST_FAILURE;
    // return MAIN_RETURN_VAL;
}
