/*
        ...
        Fourier transform of a square matrix (default size 256256) of
...
#define Float    float            /* Floating point type used in FFT */
...
static void fourn(Float data[], int nn[], const int ndim, const int isign) {
        int i1, i2, i3;
        ...
        for (idim = ndim; idim >= 1; idim--) {
           ...
           while (ifp1 < ip2) {
              ifp2 = ifp1 << 1;
              theta = isign * 6.28318530717959f / (ifp2 / ip1);
...
int main() {
        ...
        int nsize[] = {0, 0, 0};
        long fanum, fasize;
        float mapbase, mapscale, rmin, rmax, imin, imax;
