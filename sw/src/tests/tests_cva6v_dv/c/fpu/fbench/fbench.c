 ...
 1582.13                Marinchip 9900 2 Mhz, QBASIC compiler with software
  ...
  121.80                Sun 3/160 16 Mhz, Sun C.  Compiled with -fsoft switch
   ...
   11.74      26.73     Compaq Deskpro 386, 16 Mhz 80386 with 16 Mhz 80387.
    ...
    0.1128     0.2119   Dell Dimension XPS P133c, Pentium 133 MHz,
...
static float clear_aperture;

static float aberr_lspher;
static float aberr_osc;
static float aberr_lchrom;

static float max_lspher;
static float max_osc;
static float max_lchrom;

static float radius_of_curvature;
static float object_distance;
static float ray_height;
static float axis_slope_angle;
static float from_index;
static float to_index;

static float spectral_line[9];
static float s[max_surfaces][5];
static float od_sa[2][2];
...
static void transit_surface() {
        ...
        if (paraxial) {
           if (radius_of_curvature != 0.f) {
              if (object_distance == 0.f) {
                 axis_slope_angle = 0.f;
        ...
        if (radius_of_curvature != 0.f) {
           ...
           axis_slope_angle = axis_slope_angle +
              iang - asinf(rang_sin);
           sagitta = sinf((old_axis_slope_angle + iang) / 2.f);
           sagitta = 2.f * radius_of_curvature*sagitta*sagitta;
           object_distance = ((radius_of_curvature * sinf(
              old_axis_slope_angle + iang)) *
              cotf(axis_slope_angle)) + sagitta;
           return;
        }

        rang = -asinf((from_index / to_index) *
           sinf(axis_slope_angle));
        object_distance = object_distance * ((to_index *
           cosf(-rang)) / (from_index *
           cosf(axis_slope_angle)));
        axis_slope_angle = -rang;
}

/*  Perform ray trace in specific spectral line  */
static void trace_line(int line, float ray_h)
{
        int i;
        ...
        for (i = 1; i <= current_surfaces; i++) {
           ...
           if (to_index > 1.f)
              to_index = to_index + ((spectral_line[4] -
                 spectral_line[line]) /
                 (spectral_line[3] - spectral_line[6])) * ((s[i][2] - 1.f) /
                 s[i][3]);
           transit_surface();
           from_index = to_index;
...
{
        int i, j, k, errors;
        int   intr_clear_aperture;
        float od_fline, od_cline;
        #ifdef ACCURACY
        long passes;
        #endif
...
//            niter = atoi(argv[1]);
//            if (*argv[1] == '-' || niter < 1) {
//               print_str("This is John Walker's floating point accuracy and\n");
//               print_str("\nfbench <itercount>\n\n");
//               print_str("where <itercount> is the number of iterations\n");
//               print_str("to be executed.  Archival timings should be made\n");
//               print_str("with the iteration count set so that roughly five\n");
//               print_str("minutes of execution is timed.\n");
//               //exit(0);
//               return 0;
//            }
//         }
        ...
        clear_aperture = 4.f;
        intr_clear_aperture = (int)clear_aperture;
        current_surfaces = 4.f;
        ...
        for (itercount = 0; itercount < niter; itercount++) {
           ...
           for (paraxial = 0; paraxial <= 1; paraxial++) {
              ...
              trace_line(4, intr_clear_aperture/2);
           ...
           trace_line(3, intr_clear_aperture/2);
           od_cline = object_distance;

           ///* Trace marginal ray in F */

           trace_line(6, intr_clear_aperture/2);
           od_fline = object_distance;

           aberr_lspher = od_sa[1][0] - od_sa[0][0];
           aberr_osc = 1.f - (od_sa[1][0] * od_sa[1][1]) /
              (sinf(od_sa[0][1]) * od_sa[0][0]);
           aberr_lchrom = od_fline - od_cline;
           max_lspher = sinf(od_sa[0][1]);

           /* D light */

           max_lspher = 0.0000926f/(max_lspher * max_lspher);
           max_osc = 0.0025;
