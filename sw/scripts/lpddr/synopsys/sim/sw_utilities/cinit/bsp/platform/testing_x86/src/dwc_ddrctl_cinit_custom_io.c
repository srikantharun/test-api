...
typedef struct cinit_io_functions_s {
  void      (*write32)(uint64_t address, uint32_t data);
  uint32_t  (*read32) (uint64_t address);
  void      (*signal_tb) (uint32_t sig_type);
 } cinit_io_functions_t;


static cinit_io_functions_t _io_functs = { NULL, NULL, NULL};

void cinit_set_io_functions(void (*write32)(uint64_t, uint32_t),
                            uint32_t (*read32)(uint64_t),
                            void (*signal_tb)(uint32_t))
{
    _io_functs.write32 = write32;
    _io_functs.read32 = read32;
    _io_functs.signal_tb = signal_tb;
}


void dwc_ddrctl_cinit_custom_io_write32(uint64_t address, uint32_t data)
{
    if (NULL != _io_functs.write32) {
        _io_functs.write32(address, data);
    }
}

uint32_t dwc_ddrctl_cinit_custom_io_read32(uint64_t address)
{
    if (NULL == _io_functs.read32) {
        return 0xCAFE0000;
    }
    return _io_functs.read32(address);
}
...
void dwc_ddrctl_cinit_sim_io_signal_tb(uint32_t sig_type)
{
    if (NULL != _io_functs.signal_tb) {
        _io_functs.signal_tb(sig_type);
    }
}

void dwc_ddrctl_cinit_sim_io_block_appl(uint32_t sig_type)
{
