...
Changes from version C-2023.06
(recommended)
...
---------  ------------------------------------------------  ------
  4905203  Mixed-mode package DRAM is not supported          Fixed
  4843841  Incorrect LPDDR4/4x ZQ Calibration for Byte-Mode  Fixed
  
Enhancements
------------
 - Update description of messageBlock field X8Mode and CDD* and RdWrPattern*/RdWrInvert to avoid confusion
 - Save PHY ZQ Calibration result for operation below 3200Mbps
 - Update CDD calculation to improve accuracy
 - Update simulation related iterations to reduce training time
...
Changes from version C-2022.10
(recommended)
...
---------
  STAR ID  Description                                                                                                     Status
---------  -------------------------------------------------------------------------------------------------------------   ------
  4747538  DRAM ZQ Calibration Might Not Be Executed During Initial Training, Initial dfi_init_start And IO Retention Exit Fixed
  4486722  Single rank system RxclkDly and VrefDac Margin may not be saved in correct location                             Fixed
  4605553  Possibility of Multi-Pstate training failure due to incorrect PLL lock sequence                                 Fixed
  4482220  The printing of PllCtrl1 and PllCtrl4 is incorrect when the total Pstate num is 2                               Fixed

Enhancements
----------------------------------------------------------------------------------------------------------
 - Improve printing for csr PllCtrl1/PllCtrl4 and CK delay value
 - Update write leveling coarse training for LP5/5X to avoid CK stop
 - Remove messageBlock field WrLvlTrainOpt[1] since wck odt never been used
 - Increase Write DQ search range to avoid sub-optimal training result
...
Changes from version C-2022.04
(recommended)
...
---------
  STAR ID  Description                                                                                   Status
---------  --------------------------------------------------------------------------------------------  --------
  4272328  Training will fail when message block field RdWrInvert is enabled in strobeless mode          Fixed
  4326063  Additional MRW for MR12 is needed when messageBlock field X8Mode is set to 0x5 in LPDDR5/5x   Fixed

Enhancements
----------------------------------------------------------------------------------------------------------
 - Update DFIMRL formula to improve training robustness
 - Optimize Read SI Friendly training when DFE is enabled to improve performance
 - Optimize PHY TxDCA training algorithm to improve performance
 - Update comment for messageBlock fields RdWrPatternA/RdWrPatternB/RdWrInvert/X8Mode to clarify the usage 
 - Save tWCK2DQO during training to support MRR snoop feature during retraining
 - Optimize CDD information printing
 - Remove redundant Pre-Charge command in Max Read Latency Training stage
...
Changes from version C-2021.10
(mandatory)
...
---------
  STAR ID  Description                                                                                   Status
---------  --------------------------------------------------------------------------------------------  --------
  3913802  PPT2 Training for DMI lane will fail when DM enabled and WR/RD DBI disabled                   Fixed
  4057156  Message block field 'BitTimeControl' could not alter the iteration number of read and write   Fixed
  4065774  Wrong CDDs are reported through message block                                                 Fixed
  4121357  CA VREF training result is not written into DRAM                                              Fixed
  4193952  Incorrect Right and Left EyeOffset calculation during TxPPT2 training                         Fixed
  3966975  C-2021.10 Training Firmware Release Note Update                                               Fixed

Enhancements
------------------------------------------------------------------------------------------------------------------------
 - Add support for DVFSC/DVFSQ
 - Add optional PHY DCA Average for Wck Always On mode
 - Update DFIMRL formula to improve training robustness
 - Update RxClk coarse/fine algorithm to improve read training performance
 - Add prefix to string file
...
------------------------------------------------------------------------------------------------------------------------
 ...
 - Update Disable2D messageBlock comments to improve the training procedure
 - Improve TxDFE training
 - Use independant seed for RxClkT/C to improve RxDCA training
 - Add missing description for messageBlock Misc[6]
 - Add new field "PPT2OffsetMargin" to avoid delay csr overflow/underflow when PPT2 is enabled
