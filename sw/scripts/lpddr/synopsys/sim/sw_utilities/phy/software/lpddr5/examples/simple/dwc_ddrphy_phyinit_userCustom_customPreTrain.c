    ...
    if ( pstate == 0 )
      {
         ...
         *  using dwc_ddrphy_phyinit_setMb (int ps, char *field, int value)
         */
        dwc_ddrphy_phyinit_setMb (phyctx, 0, "SequenceCtrl", 0x031f); // p-state 0

        /** Example to override SequenceCtrl in message block for each p-state
         *  by assigning the data structure sub-fields directly
         */
        mb1D[0].SequenceCtrl = 0x031f; // p-state 0
      ...
      {
        mb1D[1].SequenceCtrl = 0x031f;  // p-state 1
