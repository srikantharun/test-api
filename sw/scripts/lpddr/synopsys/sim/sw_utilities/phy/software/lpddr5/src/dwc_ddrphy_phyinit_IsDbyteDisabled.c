 ...
 * \param DbyteNumber DBYTE number to check
...
int dwc_ddrphy_phyinit_IsDbyteDisabled(phyinit_config_t *phyctx, int DbyteNumber)
{
	...
	if (NumCh == 1 && (mb_LPDDR5X_1D[pUserInputBasic->FirstPState].EnabledDQsChA < 1 || mb_LPDDR5X_1D[pUserInputBasic->FirstPState].EnabledDQsChA > 8 * pUserInputBasic->NumActiveDbyteDfi0)){
		dwc_ddrphy_phyinit_assert(0, " [%s] EnabledDQsChA(%d). Value must be 0 < EnabledDQsChA < pUserInputBasic->NumActiveDbyteDfi0*8 when NumCh is 1.\n", __func__, mb_LPDDR5X_1D[pUserInputBasic->FirstPState].EnabledDQsChA);
	}

	if (NumCh == 2 && pUserInputBasic->NumActiveDbyteDfi1 > 0 && (mb_LPDDR5X_1D[pUserInputBasic->FirstPState].EnabledDQsChB > 8 * pUserInputBasic->NumActiveDbyteDfi1)){
		dwc_ddrphy_phyinit_assert(0, " [%s] EnabledDQsChB(%d). Value must be 0 <= EnabledDQsChB < pUserInputBasic->NumActiveDbyteDfi1*8.\n", __func__, mb_LPDDR5X_1D[pUserInputBasic->FirstPState].EnabledDQsChB);
	}
