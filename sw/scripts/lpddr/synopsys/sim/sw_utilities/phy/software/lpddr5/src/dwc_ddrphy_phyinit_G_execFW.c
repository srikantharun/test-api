/** \file
 *  \brief Implements Step G of initialization sequence
 *  \addtogroup SrcFunc
 *  @{
 */
...
{
  uint16_t MicroContMuxSel_data = 0x1;
  ...
  dwc_ddrphy_phyinit_MicroContMuxSel_write32(MicroContMuxSel_data);
  dwc_ddrphy_phyinit_userCustom_io_write32((tAPBONLY | csr_MicroReset_ADDR), csr_ResetToMicro_MASK | csr_StallToMicro_MASK);
  dwc_ddrphy_phyinit_userCustom_io_write32((tAPBONLY | csr_MicroReset_ADDR), csr_StallToMicro_MASK);

  dwc_ddrphy_phyinit_cmnt("\n");
  dwc_ddrphy_phyinit_cmnt("// 2. Begin execution of the training firmware by setting the MicroReset CSR to 4\'b0000.\n");
  dwc_ddrphy_phyinit_userCustom_io_write32((tAPBONLY | csr_MicroReset_ADDR), 0x0);
