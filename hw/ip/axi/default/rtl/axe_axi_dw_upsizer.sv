...
module axe_axi_dw_upsizer #(
  ...
  for (genvar t = 0; unsigned'(t) < AxiMaxReads; t++) begin: gen_read_upsizer
    ...
    addr_t start_addr;
    addr_t end_addr;
    addr_t man_port_offset;
    addr_t sub_port_offset;
    ...
    always_comb begin
      ...
      start_addr      = addr_t'(0);
      end_addr        = addr_t'(0);
      man_port_offset = addr_t'(0);
      sub_port_offset = addr_t'(0);
                ...
                axi_pkg::BurstIncr: begin
                  ...
                  if (axi_pkg::axi_modifiable_bit_set(r_req_d.ar.cache)) begin
                    ...
                    if (r_req_d.ar.len != '0) begin
                      ...
                      start_addr = addr_t'(axi_pkg::axi_aligned_addr(r_req_d.ar.addr, AxiManPortMaxSize));
                      end_addr   = addr_t'(axi_pkg::axi_aligned_addr(axi_pkg::axi_beat_addr(r_req_d.ar.addr,
        ...
        R_PASSTHROUGH, R_INCR_UPSIZE: begin
          ...
          if (!r_req_q.ar_valid)
            if (axi_man_r_valid && (idx_r_upsizer == t) && r_upsizer_valid) begin
              man_port_offset = AxiManPortStrbWidth == 1 ? '0 : r_req_q.ar.addr[cc_math_pkg::index_width(AxiManPortStrbWidth)-1:0];
              sub_port_offset = AxiSubPortStrbWidth == 1 ? '0 : r_req_q.ar.addr[cc_math_pkg::index_width(AxiSubPortStrbWidth)-1:0];
  ...
  addr_t man_port_offset_w;
  addr_t sub_port_offset_w;
  addr_t start_addr_w;
  addr_t end_addr_w;
  ...
  always_comb begin
    ...
    man_port_offset_w = addr_t'(0);
    sub_port_offset_w = addr_t'(0);
    start_addr_w      = addr_t'(0);
    end_addr_w        = addr_t'(0);
      ...
      W_PASSTHROUGH, W_INCR_UPSIZE: begin
        ...
        if (!w_req_q.aw_valid) begin
          ...
          if (i_axi_s_w_valid && o_axi_s_w_ready) begin
            man_port_offset_w = AxiManPortStrbWidth == 1 ? '0 : w_req_q.aw.addr[cc_math_pkg::index_width(AxiManPortStrbWidth)-1:0];
            sub_port_offset_w = AxiSubPortStrbWidth == 1 ? '0 : w_req_q.aw.addr[cc_math_pkg::index_width(AxiSubPortStrbWidth)-1:0];

            // Serialization
            for (int unsigned b = 0; b < AxiManPortStrbWidth; b++)
              if ((b >= man_port_offset_w) &&
                  (b - man_port_offset_w < (1 << w_req_q.orig_aw_size)) &&
                  (b + sub_port_offset_w - man_port_offset_w < AxiSubPortStrbWidth)) begin
                w_data[b]         = i_axi_s_w.data[8*(b + sub_port_offset_w - man_port_offset_w) +: 8];
                w_req_d.w.strb[b] = i_axi_s_w.strb[b + sub_port_offset_w - man_port_offset_w];
              end
          ...
          axi_pkg::BurstIncr: begin
            ...
            if (axi_pkg::axi_modifiable_bit_set(i_axi_s_aw.cache))
              ...
              if (i_axi_s_aw.len != '0) begin
                // Evaluate output burst length
                start_addr_w = addr_t'(axi_pkg::axi_aligned_addr(i_axi_s_aw.addr, AxiManPortMaxSize));
                end_addr_w   = addr_t'(axi_pkg::axi_aligned_addr(axi_pkg::axi_beat_addr(i_axi_s_aw.addr,
                    i_axi_s_aw.size, i_axi_s_aw.len, i_axi_s_aw.burst, i_axi_s_aw.len),
                    AxiManPortMaxSize));

                w_req_d.aw.len  = (end_addr_w - start_addr_w) >> AxiManPortMaxSize;
