/**
 * Abstract:
 * axi_wr_rd_sequence is used by the test to perform a write followed by a read.
...
`ifndef GUARD_AXI_WR_RD_SEQUENCE_SV
`define GUARD_AXI_WR_RD_SEQUENCE_SV

class axi_wr_rd_sequence extends axi_basic_sequence;
    ...
    virtual task body();
        ...
        for(int i = 0; i < sequence_length; i++) begin
            ...
            u_axe_clk_if.await_rising_edge(calc_delay(max_delay_between_wr_rd, min_delay_between_wr_rd));
            ...
            if(enable_checks) begin
                foreach (write_tran.data[i]) begin
                    assert(rsp.data[i] == write_tran.data[i]) begin
                        checks++;
                    end
                        else `uvm_error("RW_Seq", "Assertion failed: write and read data don't match");
                end
            end
...
endclass: axi_wr_rd_sequence

`endif // GUARD_AXI_WR_RD_SEQUENCE_SV
