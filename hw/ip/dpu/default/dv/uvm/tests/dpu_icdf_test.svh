`ifndef DPU_ICDF_TEST_SVH 
`define DPU_ICDF_TEST_SVH
...
*/
class dpu_icdf_test extends dpu_base_test;

  /** UVM Component Utility macro */
  `uvm_component_utils(dpu_icdf_test)
  typedef string string_queue_t [$];
  // Take ICDF's tests path whichs is used in dpu_icdf_test
  string ICDF_TEST_PATH = $test$plusargs("ICDF_EXTENSIVE_TEST") ? "/hw/dv/icdf/stimuli/dpu/extensive_tests/" : "/hw/dv/icdf/stimuli/dpu/quick_tests/";
  string icdf_cmd       = $sformatf("ls -d -1 \"$REPO_ROOT%s\"**\/ > icdf_test.txt", ICDF_TEST_PATH );

  string_queue_t                    path;
  axi_stream_master_file_sequence   data_seq_ifd0;
  axi_stream_master_file_sequence   data_seq_ifd1;
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    data_seq_ifd0 = axi_stream_master_file_sequence::type_id::create("data_seq_ifd0", this);
    data_seq_ifd1 = axi_stream_master_file_sequence::type_id::create("data_seq_ifd1", this);
    ...
    uvm_config_db#(string)::set(this, "env.axi_system_env.master[0].sequencer.cfg_seq", "filename", "dpu_cmd");
    uvm_config_db#(string)::set(this, "env.axi_system_env.master[1].sequencer.data_seq_ifd0", "filename", "dpu_in0");
    uvm_config_db#(string)::set(this, "env.axi_system_env.master[2].sequencer.data_seq_ifd1", "filename", "dpu_in1");
    /** Set ICDF expected output data to axi_scoreboard */
    uvm_config_db#(string)::set(null, "*", "output_filename", "dpu_out0");
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    foreach (path[i]) begin
      ...
      uvm_config_db#(string)::set(this, "uvm_test_top.data_seq_ifd0", "path", path[i]);
      uvm_config_db#(string)::set(this, "uvm_test_top.data_seq_ifd1", "path", path[i]);
      ...
      fork
        ...
        data_seq_ifd0.start(env.axi_system_env.master[1].sequencer);
        data_seq_ifd1.start(env.axi_system_env.master[2].sequencer);
      join
