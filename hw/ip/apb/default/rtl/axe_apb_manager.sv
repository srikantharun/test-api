...
/// i_apb_valid will trigger the generation of a new transaction if o_apb_ready is 1.
/// i_apb_wr_req=1 will trigger a write transaction, i_apb_wr_req=0 will trigger a read transaction.
module axe_apb_manager #(
  ...
  input  axe_apb_pkg::apb_prot_t      i_apb_pprot                ,
...
always_comb begin
  ...
  unique case(state_q)
    ...
    APB_ENABLE  : begin
      if (i_apb_m_pready) state_d = APB_IDLE;
      else                state_d = APB_ENABLE;
    end
    default : begin
      state_d = APB_IDLE;
    end
