...
//
class ai_core_ral#(int ADDR_W=40, int DATA_W=64) extends uvm_component;
    `uvm_component_param_utils(ai_core_ral#(ADDR_W, DATA_W))

  typedef bit[ADDR_W-1:0] addr_t;
  typedef bit[DATA_W-1:0] data_t;

  // Register model Generic
  register_access #(uvm_reg_block, ADDR_W, DATA_W) m_reg_blocks[`AI_CORE_REGMOD_NUM]; 
  ai_core_top_reg_block                            m_regmodel;

  function new(string name="ai_core_ral", uvm_component parent);
    super.new(name, parent);
  endfunction

  function void build_phase(uvm_phase phase);
    super.build_phase(phase);
    for (int i=0; i < `AI_CORE_REGMOD_NUM; i++) begin
  ...
  function void connect_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(ai_core_top_reg_block)::get(this, "", "ai_core_top_regmodel", m_regmodel)) begin
  ...
  function void get_register(int regblock_num, string name = "", addr_t addr = 0, output uvm_reg curr_reg);
    ...
    `uvm_info(get_name(), $sformatf("RAL access to register block %0d %s", regblock_num, regname), UVM_FULL)
  endfunction
