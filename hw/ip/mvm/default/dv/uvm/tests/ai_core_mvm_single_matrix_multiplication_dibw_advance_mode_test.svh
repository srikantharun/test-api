...
`ifndef __AI_CORE_MVM_SINGLE_MATRIX_MULTIPLICATION_DIBW_ADVANCE_MODE_TEST__
`define __AI_CORE_MVM_SINGLE_MATRIX_MULTIPLICATION_DIBW_ADVANCE_MODE_TEST__

class ai_core_mvm_single_matrix_multiplication_dibw_advance_mode_test extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_single_matrix_multiplication_dibw_advance_mode_test)
  // Class constructor
  function new (string name="ai_core_mvm_single_matrix_multiplication_dibw_advance_mode_test", uvm_component parent);
    super.new (name, parent);
  endfunction : new
  // Build phase
  virtual function void build_phase(uvm_phase phase);
    `uvm_info ("ai_core_mvm_single_matrix_multiplication_dibw_advance_mode_test", "Build phase entered", UVM_HIGH)
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    super.run_phase(phase);
      `uvm_info("MVM_single_TEST",$sformatf("MVM ai_core_mvm_single_matrix_multiplication_dibw_advance_mode_test matrix multiplication starting"), UVM_LOW)
      ...
      if(!axi_wr_seq.randomize() with {
        ...
        cfg_data[1] == 64'h0000_0101_0000_0000; //2x2 matrix programmed and weight set 0 and extra=3 (advance mode) 
      })
      ...
      if(!axi_wr_seq.randomize() with {
        ...
        burst_lenght_enum_t == BURST_LENGTH_2;
        cfg_data[0] == 64'h0000_0000_0000_0000;
        cfg_data[1] == 64'h0000_0000_0400_0000; //1x1 matrix programmed and weight set 0 and extra=3 (advance mode) 
      })
        `uvm_fatal("run_phase", "Failed to randomize");
      axi_wr_seq.start(env.axi_system_env.master[0].sequencer);
     ...
     uvm_report_info(get_type_name(), $psprintf("IFDW second PRG STATUS READ Done axi_master_stream_ifdw_base_seq = \n %s", axi_master_stream_ifdw_base_seq.sprint()), UVM_LOW);
      ...
      if(!axi_wr_seq.randomize() with {
        ...
        burst_lenght_enum_t == BURST_LENGTH_2;
        cfg_data[0] == 64'h0000_0000_0000_0000;
        cfg_data[1] == 64'h0300_0003_0002_0000;
      }) 
        `uvm_fatal("run_phase", "Failed to randomize");
      axi_wr_seq.start(env.axi_system_env.master[0].sequencer);
...
endclass:ai_core_mvm_single_matrix_multiplication_dibw_advance_mode_test

`endif	// __AI_CORE_MVM_SINGLE_MATRIX_MULTIPLICATION_DIBW_ADVANCE_MODE_TEST__
