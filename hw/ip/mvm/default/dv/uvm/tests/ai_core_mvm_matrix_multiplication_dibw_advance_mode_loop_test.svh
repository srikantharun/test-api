...
`ifndef __AI_CORE_MVM_MATRIX_MULTIPLICATION_DIBW_ADVANCE_MODE_LOOP_TEST__
`define __AI_CORE_MVM_MATRIX_MULTIPLICATION_DIBW_ADVANCE_MODE_LOOP_TEST__
...
class ai_core_mvm_matrix_multiplication_dibw_advance_mode_loop_test extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_matrix_multiplication_dibw_advance_mode_loop_test)
  ...
  int mvm_iterations, loop_idx_cmb,loop_len_idx;
  ...
  //// Class constructor
  function new (string name="ai_core_mvm_matrix_multiplication_dibw_advance_mode_loop_test", uvm_component parent);
    super.new (name, parent);
  endfunction : new
  // Build phase
  virtual function void build_phase(uvm_phase phase);
    `uvm_info ("ai_core_mvm_matrix_multiplication_dibw_advance_mode_loop_test", "Build phase entered", UVM_HIGH)
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    if(!$value$plusargs("LOOP_IDX_CMB=%0d", loop_idx_cmb)) loop_idx_cmb = 1;
    
    `uvm_info("MVM_single_TEST",$sformatf("MVM_ITERATION is %0d and loop_idx_cmb is %0d", mvm_iterations,loop_idx_cmb ), UVM_LOW)
     
    for(int i=0; i< mvm_iterations; i++) begin 
    
      void'(randomize());

      `uvm_info("MVM_single_TEST",$sformatf("MVM ai_core_mvm_matrix_multiplication_dibw_advance_mode_loop_test matrix multiplication starting, MVM_ITERATION is %0d",i), UVM_LOW)
      ...
      for ( int loop_len_idx=0; loop_len_idx<loop_idx_cmb; loop_len_idx++) begin
        `uvm_info("MVM_single_TEST",$sformatf("value inside for loop - loop_len_idx is %0d",loop_len_idx ), UVM_LOW)
        ...
        if(!axi_wr_seq.randomize() with {
          cfg_addr        == MVM_EXE_INSTR_START_ADDR + loop_len_idx*4;
          sequence_length == 'd1;
          ...
          `uvm_fatal("run_phase", "Failed to randomize");
        axi_wr_seq.start(env.axi_system_env.master[0].sequencer);
        uvm_report_info(get_type_name(), $psprintf("IFD0 exe_seq is = \n %s", axi_wr_seq.sprint()), UVM_LOW);
        ...
        if (!mvm_exe_instr_tb_data_h.randomize with {mvm_exe_instr_struct.wb_t_pw == col_len_ifd2;
          ...
          `uvm_info(get_type_name(), $psprintf("mvm_exe_instr_tb_data_h = \n %s", mvm_exe_instr_tb_data_h.sprint()), UVM_HIGH);
        if(!axi_wr_seq.randomize() with {
          cfg_addr        == MVM_EXE_INSTR_START_ADDR + (loop_len_idx*4)+2; //second addr for ifd2
          sequence_length == 'd1;
          burst_size_enum_t == BURST_SIZE_16BIT;
          burst_type_enum_t == FIXED;
          burst_lenght_enum_t == BURST_LENGTH_1;
          ...
          `uvm_fatal("run_phase", "Failed to randomize");
        axi_wr_seq.start(env.axi_system_env.master[0].sequencer);
        uvm_report_info(get_type_name(), $psprintf("IFD2 exe_seq is = \n %s", axi_wr_seq.sprint()), UVM_LOW);

      end

      if (!mvm_exe_cmd_tb_data_h.randomize with {mvm_exe_cmd_struct.loop_ptr == 0;
					                                       mvm_exe_cmd_struct.loop_len == 2*loop_idx_cmb; // utilising first two instrution
      ...
      fork 
        begin 
          if(!axi_master_stream_ifd0_base_seq.randomize() with {
            sequence_length == 1;
            burst_length == mvm_exe_cmd_tb_data_h.mvm_exe_cmd_struct.loop_iter * (row_len_ifd0+1) * loop_idx_cmb;
          })
            `uvm_fatal("run_phase", "Failed to randomize");
        ...
        begin
          if(!axi_master_stream_ifd2_base_seq.randomize() with {
            sequence_length == 1;
            burst_length == mvm_exe_cmd_tb_data_h.mvm_exe_cmd_struct.loop_iter * (row_len_ifd2+1) * loop_idx_cmb;
          })
            `uvm_fatal("run_phase", "Failed to randomize");
...
endclass:ai_core_mvm_matrix_multiplication_dibw_advance_mode_loop_test

`endif	// __AI_CORE_MVM_MATRIX_MULTIPLICATION_DIBW_ADVANCE_MODE_LOOP_TEST__
