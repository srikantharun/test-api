// MVM reg_unmapped_address_access test-case class
`ifndef __AI_CORE_MVM_REG_UNMAPPED_ADDRESS_ACCESS_TEST__
`define __AI_CORE_MVM_REG_UNMAPPED_ADDRESS_ACCESS_TEST__

class ai_core_mvm_reg_unmapped_address_access_test extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_reg_unmapped_address_access_test)
      ...
      resp_type_enum resp_type_enum_t;
      axi_master_write_sequence axi_wr_seq;
      axi_master_read_sequence axi_rd_seq;
  // Class constructor
  function new (string name="ai_core_mvm_reg_unmapped_address_access_test", uvm_component parent);
    super.new (name, parent);
  endfunction : new
  // Build phase
  virtual function void build_phase(uvm_phase phase);
    `uvm_info ("ai_core_mvm_reg_unmapped_address_access_test", "Build phase entered", UVM_HIGH)
  ...
  // Address and data
      ...
      if(axi_rd_seq.read_transaction.rresp[0] == AXI_SLVERR_RESPONSE)
       `uvm_info("MVM_READ_REG_COMPARE: PASSED", $sformatf("RRESP = %s", axi_rd_seq.read_transaction.rresp[0]), UVM_LOW)
      else
       `uvm_error("MVM_READ_REG_COMPARE: FAILED", $sformatf("RRESP = %s",axi_rd_seq.read_transaction.rresp[0]))
    phase.drop_objection(this);
  endtask

endclass:ai_core_mvm_reg_unmapped_address_access_test

`endif	// __AI_CORE_MVM_REG_UNMAPPED_ADDRESS_ACCESS_TEST__
