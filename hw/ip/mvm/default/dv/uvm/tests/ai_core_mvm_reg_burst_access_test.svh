...
`ifndef __AI_CORE_MVM_REG_BURST_ACCESS_TEST__
`define __AI_CORE_MVM_REG_BURST_ACCESS_TEST__

class ai_core_mvm_reg_burst_access_test extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_reg_burst_access_test)
  ...
  // Class constructor
  function new (string name="ai_core_mvm_reg_burst_access_test", uvm_component parent);
    super.new (name, parent);
  endfunction : new
  // Build phase
  virtual function void build_phase(uvm_phase phase);
    `uvm_info ("ai_core_mvm_reg_burst_access_test", "Build phase entered", UVM_HIGH)
  ...
  // Address and data
      ...
      foreach(axi_rd_seq.read_transaction.rresp[i]) begin
        if(axi_rd_seq.read_transaction.rresp[i] == AXI_OKAY_RESPONSE)
...
endclass:ai_core_mvm_reg_burst_access_test

`endif	// __AI_CORE_MVM_REG_BURST_ACCESS_TEST__
