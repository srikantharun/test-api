...
`ifndef __AI_CORE_MVM_CONCURRENT_PRG_EXE_RAND_MATRIX_MULTIPLICATION_DIBW_SIMPLE_MODE_TEST__
`define __AI_CORE_MVM_CONCURRENT_PRG_EXE_RAND_MATRIX_MULTIPLICATION_DIBW_SIMPLE_MODE_TEST__

class ai_core_mvm_concurrent_prg_exe_rand_matrix_multiplication_dibw_simple_mode_test extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_concurrent_prg_exe_rand_matrix_multiplication_dibw_simple_mode_test)
      ...
      bit prg_wt_set;
  // Class constructor
  function new (string name="ai_core_mvm_concurrent_prg_exe_rand_matrix_multiplication_dibw_simple_mode_test", uvm_component parent);
  ...
  task wait_for_prg_status;
      do begin
        ral_read_prg_status   = 64'h000_0000;
        mvm_regmodel.m_mvmprg_regmod.cmdblk_status.read(status, ral_read_prg_status);
      end while(|ral_read_prg_status[1:0]); // IDLE state == 0
  endtask
  ...
  task prepare_exe_instr;
    ...
    if (($test$plusargs("SHORT_RUN")) ) begin
      ...
      if (!mvm_exe_instr_tb_data_packet_h.randomize())
         `uvm_fatal("run_phase", "Failed to randomize");
    end
...
endclass:ai_core_mvm_concurrent_prg_exe_rand_matrix_multiplication_dibw_simple_mode_test
