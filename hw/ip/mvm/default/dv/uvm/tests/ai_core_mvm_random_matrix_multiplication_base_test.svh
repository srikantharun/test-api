...
`ifndef __AI_CORE_MVM_RANDOM_MATRIX_MULTIPLICATION_BASE_TEST__
`define __AI_CORE_MVM_RANDOM_MATRIX_MULTIPLICATION_BASE_TEST__

class ai_core_mvm_random_matrix_multiplication_base_test extends ai_core_mvm_base_test;
  int addr_size;

  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_random_matrix_multiplication_base_test)
  ...
  // Class constructor
  function new (string name="ai_core_mvm_random_matrix_multiplication_base_test", uvm_component parent);
  ...
  virtual task run_phase(uvm_phase phase);
    super.run_phase(phase);
      if(!$value$plusargs("ADDR_SIZE=%0d", addr_size)) addr_size = 20; //restricting to 20 as it will solve the purpose of command fifo full (16)
  endtask
  ...
  task prepare_exe_instr;
    ...
    if (($test$plusargs("SHORT_RUN")) ) begin
      ...
      if (!mvm_exe_instr_tb_data_packet_h.randomize() with {mvm_exe_instr_addr.size == addr_size;} )
         `uvm_fatal("run_phase", "Failed to randomize");
...
endclass:ai_core_mvm_random_matrix_multiplication_base_test

`endif	// __AI_CORE_MVM_RANDOM_MATRIX_MULTIPLICATION_BASE_TEST__
