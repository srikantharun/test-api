// Testcase writes into the loop iter into 16 and checks the overflow using the single matrix multiplication.
`ifndef __AI_CORE_MVM_SINGLE_MATRIX_MULTIPLICATION_TEST_FIFO_OVERFLOW__
`define __AI_CORE_MVM_SINGLE_MATRIX_MULTIPLICATION_TEST_FIFO_OVERFLOW__

class ai_core_mvm_single_matrix_multiplication_test_fifo_overflow extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_single_matrix_multiplication_test_fifo_overflow)
  // Class constructor
  function new (string name="ai_core_mvm_single_matrix_multiplication_test", uvm_component parent);
    super.new (name, parent);
  endfunction : new
  // Build phase
  virtual function void build_phase(uvm_phase phase);
    `uvm_info ("ai_core_mvm_single_matrix_multiplication_test", "Build phase entered", UVM_HIGH)
  ...
  virtual task run_phase(uvm_phase phase);
   ...
   // Reproduce the issue #2147 at the block level with 5 iterations loop len 8 and one iteration with loop len 16.   
   for (int i=0; i< 5; i++) begin    
   
      `uvm_info("MVM_SINGLE_TEST",$sformatf("MVM single matrix multiplication starting %d",i), UVM_LOW)
      ...
      if (!mvm_prg_cmd_tb_data_h.randomize() with {mvm_prg_cmd_struct.wb_t_pw == 3;
      ...
      uvm_report_info(get_type_name(), $psprintf("PRG STRUCT is mvm_prg_cmd_tb_data_h = \n %s", mvm_prg_cmd_tb_data_h.sprint()), UVM_HIGH);
      ...
      if(!axi_wr_seq.randomize() with {
        ...
        cfg_data[1] == mvm_prg_cmd_tb_data_h.mvm_prg_cmd_data; //64'h0000_0308_0000_0000;
      })
      ...
      if (!mvm_exe_instr_tb_data_h.randomize() with {mvm_exe_instr_struct.wb_t_pw == 3;
					         mvm_exe_instr_struct.wb_u_pw == 8;
					         mvm_exe_instr_struct.a_t_pw ==0;
					         mvm_exe_instr_struct.a_u_pw ==0;	
   		                                 mvm_exe_instr_struct.a_s == 0;
						})
       `uvm_fatal("run_phase", "Failed to randomize the exe instr struct");
	uvm_report_info(get_type_name(), $psprintf("EXE INSTR struct is mvm_exe_instr_tb_data_h = \n %s", mvm_exe_instr_tb_data_h.sprint()), UVM_HIGH);
      ...
      if (!mvm_exe_cmd_tb_data_h.randomize() with {mvm_exe_cmd_struct.loop_ptr == 0;
					         ...
					         mvm_exe_cmd_struct.loop_iter =='h10;
					         mvm_exe_cmd_struct.extra == 0;	
						 })
	`uvm_fatal("run_phase", "Failed to randomize");
	uvm_report_info(get_type_name(), $psprintf("EXE cmd struct is mvm_exe_cmd_tb_data_h = \n %s", mvm_exe_cmd_tb_data_h.sprint()), UVM_HIGH);
      ...
      if(!axi_wr_seq.randomize() with {
        ...
        burst_lenght_enum_t == BURST_LENGTH_2;
        cfg_data[0] == 64'h0000_0000_0000_0000;
        cfg_data[1] == mvm_exe_cmd_tb_data_h.mvm_exe_cmd_data; //64'h10_0001_0000;
      }) 
        `uvm_fatal("run_phase", "Failed to randomize");
      axi_wr_seq.start(env.axi_system_env.master[0].sequencer);
...
endclass:ai_core_mvm_single_matrix_multiplication_test_fifo_overflow

`endif	// __AI_CORE_MVM_SINGLE_MATRIX_MULTIPLICATION_TEST_FIFO_OVERFLOW__
