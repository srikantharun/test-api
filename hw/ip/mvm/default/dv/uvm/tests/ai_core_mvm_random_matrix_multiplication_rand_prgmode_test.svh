...
`ifndef __AI_CORE_MVM_RANDOM_MATRIX_MULTIPLICATION_RAND_PRGMODE_TEST__
`define __AI_CORE_MVM_RANDOM_MATRIX_MULTIPLICATION_RAND_PRGMODE_TEST__

class ai_core_mvm_random_matrix_multiplication_rand_prgmode_test extends ai_core_mvm_base_test;
  // Registration with the factory
  `uvm_component_utils(ai_core_mvm_random_matrix_multiplication_rand_prgmode_test)
   bit[7:0] extra ;
  ...
  // Class constructor
  function new (string name="ai_core_mvm_random_matrix_multiplication_rand_prgmode_test", uvm_component parent);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    super.run_phase(phase);
      ...
      `uvm_info("MVM_RANDOM_TEST",$sformatf("MVM random matrix multiplication rand prgmode starting"), UVM_LOW)
      #100ns;
      ...
      if(mvm_if.prgmode == UuT | mvm_if.prgmode == TUu | mvm_if.prgmode == UTu) begin
	`uvm_info("PRGMODE",$sformatf("Programming mode set to %s", mvm_if.prgmode),UVM_LOW)
      end else begin
	`uvm_error("PRG MODE","Mismatch is prgmode") 	   
      end
     
      `uvm_info("MVM_RANDOM_TEST",$sformatf("MVM random matrix multiplication rand prgmode end"), UVM_LOW)
     ...
     for (int weight_set=0;weight_set<WEIGHT_SETS;weight_set++) begin
        ...
        if(mvm_exe_cmd_tb_data_packet_h.mvm_exe_instr_addr_sort[weight_set].size == 1) begin //  size ==1
            ...
            if (($test$plusargs("SHORT_RUN")) ) begin
              ...
              if(!mvm_exe_cmd_tb_data_h.randomize() with {mvm_exe_cmd_struct.loop_len == mvm_exe_cmd_tb_data_packet_h.len;
                                                     mvm_exe_cmd_struct.loop_ptr ==  mvm_exe_cmd_tb_data_packet_h.ptr;
                                                     mvm_exe_cmd_struct.loop_iter inside {[6:60]}; })
                 `uvm_fatal("run_phase", "Failed to randomize");
...
endclass:ai_core_mvm_random_matrix_multiplication_rand_prgmode_test

`endif	// __AI_CORE_MVM_RANDOM_MATRIX_MULTIPLICATION_RAND_PRGMODE_TEST__
