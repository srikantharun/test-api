...
/**
 * Abstract:
 * Class axi_slave_mem_delay_sequence defines a sequence class that
...
`ifndef GUARD_AXI_SLAVE_MEM_DELAY_SEQUENCE_SV
`define GUARD_AXI_SLAVE_MEM_DELAY_SEQUENCE_SV

class axi_slave_mem_delay_sequence extends svt_axi_slave_base_sequence;

  svt_axi_slave_transaction req_resp;
  int delay;

  /** UVM Object Utility macro */
  `uvm_object_utils(axi_slave_mem_delay_sequence)

  /** Class Constructor */
  function new(string name="axi_slave_mem_delay_sequence");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if($test$plusargs("IAU_READY_DELAY")) begin
      $value$plusargs("IAU_READY_DELAY=%0d",delay);
      `uvm_info("Set-Op: TEST", $sformatf("value of delay = %d ", delay), UVM_HIGH)
    end
    else delay = 8;
    ...
    forever begin
      ...
      status=req_resp.randomize with {
        ...
        //  tready_delay[i]== delay; 
        //};
       foreach (tready_delay[i]) {
                           if (i == 0) 
                           tready_delay[i] == delay ;
                           else tready_delay[i] == 0;
                         }
       };
       if(!status)
...
endclass: axi_slave_mem_delay_sequence

`endif // GUARD_AXI_SLAVE_MEM_DELAY_SEQUENCE_SV
