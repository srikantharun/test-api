...
class cust_svt_axi_system_configuration extends svt_axi_system_configuration;
  ...
  `ifdef SLAVE_AGENT
    this.num_slaves  = 1;
    if (($test$plusargs("PROTOCOL_CHECKER_DISABLE_FOR_SLEEP_MODE")) ) begin
  ...
  `endif
    ...
    this.master_cfg[0].data_width = 512;
    this.master_cfg[0].addr_width = 24;
    this.master_cfg[0].id_width   = 3;
    this.master_cfg[0].num_outstanding_xact=4;
    if ($test$plusargs("BCK_BCK_TRANS"))
    begin
    this.master_cfg[0].default_bready = 1;
    this.master_cfg[0].default_rready = 1;
    end
    if($test$plusargs("STRESS_CHECK "))
    begin
    this.master_cfg[0].default_bready = 0;
    this.master_cfg[0].default_rready = 0;
    end
  ...
  `ifdef SLAVE_AGENT
    ...
    this.slave_cfg[0].addr_width = 24;
    this.slave_cfg[0].id_width   = 3;
    this.slave_cfg[0].num_outstanding_xact=4;
    if ($test$plusargs("BCK_BCK_TRANS"))
    begin
        this.slave_cfg[0].default_awready = 1;
    this.slave_cfg[0].default_wready = 1; 
    this.slave_cfg[0].default_arready = 1;
    this.slave_cfg[0].default_rready = 1;
    end
    if (($test$plusargs("PROTOCOL_CHECKER_DISABLE_FOR_SLEEP_MODE")) ) begin
  ...
  `endif
    ...
    `else
      this.set_addr_range(0,24'h0,24'hff_ffff);
    `endif
  endfunction

  `ifdef ENABLE_COMPLEX_MEMORY_MAP_FEATURE
    function bit get_dest_slave_addr_from_global_addr (chip_axi_addr_t global_addr,
