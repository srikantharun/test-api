...
class axi_write_sequence extends svt_axi_master_base_sequence;
 ...
 //Utility and Field macros,
  ...
  virtual task body();
    ...
    bit status;
    `uvm_info("axi_write_sequence: Body", "Entered", UVM_LOW)
    ...
    if (!$cast(cfg, get_cfg)) begin
      `uvm_fatal("axi_write_sequence: Body", "Unable to $cast the configuration to a svt_axi_port_configuration class");
    end
    ...
    for (int i = 0; i < sequence_length; i++) begin
      ...
      write_tran.burst_length = int'(burst_length_t);
      ...
      foreach (write_tran.wstrb[i]) begin
        for (int j = 0; j < write_tran.burst_size; j++) begin
          write_tran.wstrb[i] = 1'b1;
        end
      end
      ...
      write_transaction = write_tran;
           uvm_report_info(get_type_name(), $psprintf("axi_write_sequence: Body write_transaction = \n %s", write_transaction.sprint()), UVM_LOW);
    end
    `uvm_info("axi_write_sequence: Body", "Exiting", UVM_LOW)
  endtask : body

endclass : axi_write_sequence

// AXI master read sequence
class axi_read_sequence extends svt_axi_master_base_sequence;
  ...
  virtual task body();
    ...
    svt_configuration get_cfg;
    bit status;
    `uvm_info("axi_read_sequence: Body", "Entered", UVM_LOW)
    ...
    p_sequencer.get_cfg(get_cfg);
    if (!$cast(cfg, get_cfg)) begin
      `uvm_fatal("axi_read_sequence: Body", "Unable to $cast the configuration to a svt_axi_port_configuration class");
    end
    ...
    for (int i = 0; i < sequence_length; i++) begin
      ...
      read_tran.burst_length = int'(burst_length_t);
    ...
    uvm_report_info(get_type_name(), $psprintf("axi_read_sequence: Body read_transaction = \n %s", read_transaction.sprint()), UVM_LOW);
    `uvm_info("axi_read_sequence: Body", "Exiting", UVM_LOW)
  endtask : body
