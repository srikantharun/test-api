    ...
    forever begin
     ...
     /**
      ...
      status=req_resp.randomize with {
       ...
       if($test$plusargs("INTRLV_TEST")) begin
        $value$plusargs("INTRLV_TEST=%0d",intrlv);
         req_resp.interleave_pattern = svt_axi_slave_transaction::RANDOM_BLOCK;
         req_resp.enable_interleave  = 1;
        end
      ...
      if(req_resp.get_transmitted_channel() == svt_axi_slave_transaction::WRITE) begin
        ...
        `uvm_info("body", $psprintf(" 1111111111111111 slave_mem_response write Addr=%0h dataaa=%0h",req_resp.addr,req_resp.data[0]), UVM_LOW)
        `endprotect
      end
      else if (req_resp.get_transmitted_channel() == svt_axi_slave_transaction::READ) begin
        `protect
        get_read_data_from_mem_to_transaction(req_resp);
        `uvm_info("body", $psprintf(" 1111111111111111 slave_mem_response read Addr=%0h dataaa=%0h",req_resp.addr,req_resp.data[0]), UVM_LOW)
        `endprotect
      end
