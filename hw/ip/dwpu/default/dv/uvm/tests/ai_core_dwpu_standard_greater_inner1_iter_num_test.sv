/**
* Generate a program to test the inner1 number of iterations greater than the sanity values.
* Random Configure dwpu_dp's  signed of weight and image regs
* Send data stream accordingly generated program requests
*/
class ai_core_dwpu_standard_greater_inner1_iter_num_test extends ai_core_dwpu_standard_test;

  /** UVM Component Utility macro */
  `uvm_component_utils (ai_core_dwpu_standard_greater_inner1_iter_num_test)
  ...
  virtual function void randomize_cmd_seq(ai_core_dwpu_cmd_sequence cmd_seq, int iter, cmd_cfg_st cmd_cfg);
    ...
    if(!cmd_seq.randomize() with {
      ...
      if(iter inside {1,3}) cmd_loop_info.inner1_iter == 1;
      else                  cmd_loop_info.inner1_iter == ((1<<DWPU_DW_INNER1_ITER)-1);
      loop_cfg.has_loop1  == 1;
  ...
  endtask
endclass : ai_core_dwpu_standard_greater_inner1_iter_num_test
