/**
* Generate a program to test the outer number of iterations greater than the sanity values.
* Random Configure dwpu_dp's  signed of weight and image regs
* Send data stream accordingly generated program requests
*/
class ai_core_dwpu_standard_greater_outer_iter_num_test extends ai_core_dwpu_standard_test;

  /** UVM Component Utility macro */
  `uvm_component_utils (ai_core_dwpu_standard_greater_outer_iter_num_test)
  ...
  virtual function void randomize_cmd_seq(ai_core_dwpu_cmd_sequence cmd_seq, int iter, cmd_cfg_st cmd_cfg);
    ...
    if(!cmd_seq.randomize() with {
      ...
      if(iter inside {1,3}) cmd_loop_info.outer_iter  == 1;
      else                  cmd_loop_info.outer_iter  ==  ((1<<DWPU_DW_OUTER_ITER)-1);
      //to speed up the simulation the inner0 and inner1 iteration number is set to 0
      cmd_loop_info.inner0_iter == 0;
      cmd_loop_info.inner1_iter == 0;
      loop_cfg.has_outer  == 1;
  ...
  virtual task run_phase(uvm_phase phase);
    data_stream_cfg_t data_stream_len;
  ...
  endtask
endclass : ai_core_dwpu_standard_greater_outer_iter_num_test
