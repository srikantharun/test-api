/**
* Generate a program to test the init number of iterations greater than the sanity values.
* Random Configure dwpu_dp's  signed of weight and image regs
* Send data stream accordingly generated program requests
*/
class ai_core_dwpu_standard_greater_init_iter_num_test extends ai_core_dwpu_standard_test;

  /** UVM Component Utility macro */
  `uvm_component_utils (ai_core_dwpu_standard_greater_init_iter_num_test)
  ...
  virtual function void randomize_cmd_seq(ai_core_dwpu_cmd_sequence cmd_seq, int iter, cmd_cfg_st cmd_cfg);
    ...
    if(!cmd_seq.randomize() with {
      ...
      if(iter inside {1,3}) cmd_loop_info.init_iter == 1;
      else                  cmd_loop_info.init_iter == ((1<<DWPU_DW_INIT_ITER)-1);
      loop_cfg.has_init   == 1;
  ...
  endtask
endclass : ai_core_dwpu_standard_greater_init_iter_num_test
