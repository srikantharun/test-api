 ...
 */
class ai_core_dwpu_cmd_tr extends ai_core_dp_cmd_gen_uvm_pkg::ai_core_dp_cmd_gen_cmdb;
  
  dwpu_cmd_header_t header;

  `uvm_object_utils_begin(ai_core_dwpu_cmd_tr)
    `uvm_field_int(  header,         UVM_DEFAULT | UVM_NOPRINT)
  `uvm_object_utils_end

  function new(string name = "ai_core_dwpu_cmd_tr");
    super.new(name);
  endfunction

  extern function void do_print(uvm_printer printer);
  extern function void do_copy_from_base(ai_core_dp_cmd_gen_uvm_pkg::ai_core_dp_cmd_gen_cmdb aux);
endclass : ai_core_dwpu_cmd_tr

function void ai_core_dwpu_cmd_tr::do_print(uvm_printer printer);
...
function void ai_core_dwpu_cmd_tr::do_copy_from_base(ai_core_dp_cmd_gen_uvm_pkg::ai_core_dp_cmd_gen_cmdb aux);
  super.do_copy(aux);
