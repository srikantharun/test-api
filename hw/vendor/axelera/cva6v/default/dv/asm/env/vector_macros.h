...
#define read_vlenb(buf)                                   \
  do {                                                    \
    asm volatile("csrr %[BUF], vlenb" : [BUF] "=r"(buf)); \
  } while (0);
...
#define read_vxsat(buf)                                   \
  do {                                                    \
    asm volatile("csrr %[BUF], vxsat" : [BUF] "=r"(buf)); \
  } while (0);
#define reset_vxsat                           \
  do {                                        \
    asm volatile("csrw vxsat, %0" ::"rK"(0)); \
  } while (0);
#define set_vxrm(val)                          \
  do {                                         \
    asm volatile("csrw vxrm, %0" ::"rK"(val)); \
  } while (0);
...
// illegal values of vlmul and vsew which violate: ELEN >= SEW/LMUL
#define check_vtype_vl(casenum, vtype, golden_vtype, avl, vl, vsew, vlmul)                                         \
  verbose_printf("Checking vtype and vl #%d...\n", casenum);                                                       \
  uint64_t vlenb, vlen;                                                                                            \
  read_vlenb(vlenb);                                                                                               \
  vlen                 = vlenb * 8;                                                                                \
  uint64_t actual_vlen = vlmul & 0x4 ? vlen >> ((~vlmul & 0x3) + 1) : vlen << vlmul;                               \
  uint64_t golden_vl   = actual_vlen / (8 << vsew);                                                                \
  if (avl < golden_vl) golden_vl = avl;                                                                            \
  if (vsew == 3 || vlmul == 4 || vlmul == 5 || (vlmul == 6 && (vsew == 1 || vsew == 2)) ||                         \
      (vlmul == 7 && vsew == 2)) {                                                                                 \
    if ((vtype != 0x8000000000000000) || (vl != 0)) {                                                              \
      printf(                                                                                                      \
          "Checking vtype and vl #%d... FAILED. Got vtype = %lx, expected vtype = 8000000000000000. vl = %lx, "    \
          "expected vl = %lx.\n",                                                                                  \
          casenum, vtype, vl, avl);                                                                                \
      return;                                                                                                      \
    }                                                                                                              \
  } else if (vtype != golden_vtype || vl != golden_vl) {                                                           \
    printf(                                                                                                        \
        "Checking vtype and vl #%d... FAILED. Got vtype = %lx, expected vtype = %lx. Got vl = %lx, expected vl = " \
        "%lx.\n",                                                                                                  \
        casenum, vtype, golden_vtype, vl, golden_vl);                                                              \
    num_failed++;                                                                                                  \
    return;                                                                                                        \
  }                                                                                                                \
  verbose_printf("PASSED.\n");

#define check_vxsat(casenum, vxsat, golden_vxsat)                                                                  \
  verbose_printf("Checking vxsat #%d...\n", casenum);                                                              \
  if (vxsat != golden_vxsat) {                                                                                     \
    printf("Checking vxsat #%d... FAILED. Got vxsat = %lx, expected vxsat = %x.\n", casenum, vxsat, golden_vxsat); \
    num_failed++;                                                                                                  \
    return;                                                                                                        \
  }                                                                                                                \
  verbose_printf("PASSED.\n");
...
#define EXIT_CHECK()                                   \
  do {                                                 \
    MEMORY_BARRIER;                                    \
    if (num_failed > 0) {                              \
      printf("ERROR: failed %d tests!\n", num_failed); \
      return num_failed;                               \
    } else {                                           \
      verbose_printf("Passed all test cases!\n");      \
      return 0;                                        \
    }                                                  \
  } while (0);
...
#define XCMP(casenum, act, exp)                                                                  \
  verbose_printf("Checking the results of the test case %d:\n", casenum);                        \
  if (act != exp) {                                                                              \
    printf("Test case %d: Index %d FAILED. Got %d, expected %d.\n", casenum, casenum, act, exp); \
    num_failed++;                                                                                \
    return;                                                                                      \
  }                                                                                              \
  verbose_printf("PASSED.\n");
...
#define FCMP(casenum, act, exp)                                                                    \
  if (act != exp) {                                                                                \
    printf("Test case %d: Index %d FAILED. Got %lf, expected %lf.\n", casenum, casenum, act, exp); \
    num_failed++;                                                                                  \
    return;                                                                                        \
  }
...
#define VCMP(T, str, casenum, vexp, act...)                                                                        \
  ...
  for (unsigned int i = 0; i < sizeof(vact) / sizeof(T); i++) {                                                    \
    if (vexp[i] != vact[i]) {                                                                                      \
      printf("Test case %d: Index %d FAILED. Got %" #str ", expected %" #str ".\n", casenum, i, vexp[i], vact[i]); \
      num_failed++;                                                                                                \
      return;                                                                                                      \
    }                                                                                                              \
  }                                                                                                                \
...
// Macro to enable and disable tests that masked instructions
#define MASKED_TEST(TEST) \
  ({                      \
    if (ENABLE_MASKED) {  \
      TEST;               \
    }                     \
...
#define VCMP_U64(casenum, vect, act...)   \
  {                                       \
    VSTORE_U64(vect);                     \
    VCMP(uint64_t, x, casenum, Ru64, act) \
  }
#define VCMP_U32(casenum, vect, act...)   \
  {                                       \
    VSTORE_U32(vect);                     \
    VCMP(uint32_t, x, casenum, Ru32, act) \
  }
#define VCMP_U16(casenum, vect, act...)   \
  {                                       \
    VSTORE_U16(vect);                     \
    VCMP(uint16_t, x, casenum, Ru16, act) \
  }
#define VCMP_U8(casenum, vect, act...)  \
  {                                     \
    VSTORE_U8(vect);                    \
    VCMP(uint8_t, x, casenum, Ru8, act) \
  }

#define LVCMP_U8(casenum, vect, act)         \
  {                                          \
    uint64_t vl;                             \
    read_vl(vl);                             \
    VSTORE_L8(vect);                         \
...
#define LVCMP_U16(casenum, vect, act)          \
  {                                            \
    uint64_t vl;                               \
    read_vl(vl);                               \
    VSTORE_L16(vect);                          \
...
#define LVCMP_U32(casenum, vect, act)          \
  {                                            \
    uint64_t vl;                               \
    read_vl(vl);                               \
    VSTORE_L32(vect);                          \
...
#define LVCMP_U64(casenum, vect, act)          \
  {                                            \
    uint64_t vl;                               \
    read_vl(vl);                               \
    VSTORE_L64(vect);                          \
...
#define VVCMP_U64(casenum, ptr64, act...) \
  { VCMP(uint64_t, x, casenum, ptr64, act) }
#define VVCMP_U32(casenum, ptr32, act...) \
  { VCMP(uint32_t, x, casenum, ptr32, act) }
#define VVCMP_U16(casenum, ptr16, act...) \
  { VCMP(uint16_t, x, casenum, ptr16, act) }
#define VVCMP_U8(casenum, ptr8, act...) \
  { VCMP(uint8_t, x, casenum, ptr8, act) }

#define VVCMP_I64(casenum, ptr64, act...) \
  { VCMP(int64_t, x, casenum, ptr64, act) }
#define VVCMP_I32(casenum, ptr32, act...) \
  { VCMP(int32_t, x, casenum, ptr32, act) }
#define VVCMP_I16(casenum, ptr16, act...) \
  { VCMP(int16_t, x, casenum, ptr16, act) }
#define VVCMP_I8(casenum, ptr8, act...) \
  { VCMP(int8_t, x, casenum, ptr8, act) }

#define LVVCMP_U64(casenum, ptr64, act)         \
  {                                             \
    uint64_t vl;                                \
    read_vl(vl);                                \
    LVCMP(uint64_t, x, casenum, vl, ptr64, act) \
  }
#define LVVCMP_U32(casenum, ptr32, act)         \
  {                                             \
    uint64_t vl;                                \
    read_vl(vl);                                \
    LVCMP(uint32_t, x, casenum, vl, ptr32, act) \
  }
#define LVVCMP_U16(casenum, ptr16, act)         \
  {                                             \
    uint64_t vl;                                \
    read_vl(vl);                                \
    LVCMP(uint16_t, x, casenum, vl, ptr16, act) \
  }
#define LVVCMP_U8(casenum, ptr8, act)         \
  {                                           \
    uint64_t vl;                              \
    read_vl(vl);                              \
    LVCMP(uint8_t, x, casenum, vl, ptr8, act) \
  }
