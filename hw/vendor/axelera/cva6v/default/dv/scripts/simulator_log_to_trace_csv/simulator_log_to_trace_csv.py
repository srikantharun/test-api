...
sys.path.insert(0, os.path.join(os.environ['REPO_ROOT'], 'hw/vendor/axelera/cva6v/default/dv/deps/sim'))
...
def read_verilator_trace(path, full_trace, enable_debug_region=False, full_hex=False):
  ...
  end_trampoline_re = re.compile(r'core.*: 0x0000002000000000 ')
    ...
    for line in handle:
      ...
      if vector_match:
        ...
        if instr.instr_str[:4] == 'vset':
          ...
          ## This is used when RTL does not perform a writeback, but we need to fill in the previous
        ...
        logging.debug("Scalar Instruction: %s GPR: %s",   instr.instr_str, instr.gpr)
    # At EOF, we might have an instruction in hand. Yield it if so.
    if instr is not None:
      yield (instr, False)


def process_verilator_sim_log(verilator_log, csv, full_trace = False, vector = False, vector_only=False, full_hex=False):
  ...
  with open(csv, "w") as csv_fd:
    ...
    for (entry, illegal) in read_verilator_trace(verilator_log, full_trace, False, full_hex):
      instrs_in += 1
  ...
  logging.info("Output    instruction count : %d" % instrs_out)
...
def shorten_hex_notation(hex_string):
  # This function is used to trim a lot of digits from a very long number.
  # For example: 0x0^960-12345 means there are 960 leading zeroes. followed by the digits that come after "-", which
  # in this case is 0x12345
  # Validate input
...
def main():
  ...
  process_verilator_sim_log(args.log, args.csv, args.full_trace, args.vector, args.vector_only, args.full_hex)
