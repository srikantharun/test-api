...
class RiscvInstructionTraceEntry(object):
    ...
    def trim_fp_gpr_values(self, trim_size_hex=8):
        """ Trim captured trace of fp registers to desired width 
            Default of 8 corresponds to 8*4(per hex character)=32 bits
        """
        new_gpr = []
        for item in self.gpr:
            reg_name, hex_value = item.split(":")
            if reg_name[0] == "f":
                trimmed_hex_value = hex_value[-trim_size_hex:] # get 32 LSBs
                new_gpr.append(f"{reg_name}:{trimmed_hex_value}")
            else:
                new_gpr.append(item)
        self.gpr = new_gpr
