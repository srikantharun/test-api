...
void TEST_CASE3(void) {
  ...
  asm volatile("vmset.m v1");
  asm volatile("vmfle.vf v1, v2, %[A]" ::[A] "f"(fscalar_16));
  VSET(1, e16, m1);
  ...
  asm volatile("vmset.m v2");
  asm volatile("vmfle.vf v2, v4, %[A]" ::[A] "f"(fscalar_32));
  VSET(1, e16, m1);
  VCMP_U16(8, v2, 0xffff);
};
...
void TEST_CASE4(void) {
  ...
  VLOAD_8(v0, 0xAA, 0xAA);
  asm volatile("vmset.m v1");
  asm volatile("vmfle.vf v1, v2, %[A], v0.t" ::[A] "f"(fscalar_16));
  VSET(1, e16, m1);
  ...
  VLOAD_8(v0, 0xAA, 0xAA);
  asm volatile("vmset.m v2");
  asm volatile("vmfle.vf v2, v4, %[A], v0.t" ::[A] "f"(fscalar_32));
  VSET(1, e16, m1);
  VCMP_U16(11, v2, 0xfff7);
};
