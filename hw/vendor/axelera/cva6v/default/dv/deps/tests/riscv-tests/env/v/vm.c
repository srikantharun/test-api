...
#define assert(x, y) do { \
  ...
  cputstring("Assertion failed: " stringify(y) "\n"); \
  terminate(3); \
} while(0)
...
{
  assert(addr >= PGSIZE && addr < MAX_TEST_PAGES * PGSIZE, DEBUG1);
  ...
  if (node->addr)
  {
    ...
    assert(user_llpt[addr/PGSIZE] & PTE_A, DEBUG2);
    uintptr_t sstatus = set_csr(sstatus, SSTATUS_SUM);
    if (memcmp((void*)addr, uva2kva(addr), PGSIZE)) {
      assert(user_llpt[addr/PGSIZE] & PTE_D, DEBUG3);
...
{
  assert(addr >= PGSIZE && addr < MAX_TEST_PAGES * PGSIZE, DEBUG4);
  addr = addr/PGSIZE*PGSIZE;
  ...
  if (user_llpt[addr/PGSIZE]) {
    if (!(user_llpt[addr/PGSIZE] & PTE_A)) {
      ...
      assert(!(user_llpt[addr/PGSIZE] & PTE_D) && cause == CAUSE_STORE_PAGE_FAULT, DEBUG5);
  ...
  assert(user_mapping[addr/PGSIZE].addr == 0, DEBUG7);
...
# error
  ...
  if (read_csr(sptbr) != sptbr_value)
    assert(!"unsupported satp mode", DEBUG11);
