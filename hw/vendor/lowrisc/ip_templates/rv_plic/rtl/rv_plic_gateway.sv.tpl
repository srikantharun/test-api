...
module ${module_instance_name}_gateway #(
  ...
  logic [N_SOURCE-1:0] ip_o_d;
  always_comb ip_o_d = (ip_o | (set & ~ia & ~ip_o)) & (~(ip_o & claim_i));
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ip_o <= '0;
    end else begin
      if (ip_o != ip_o_d) ip_o <= ip_o_d;
    end
  end
  ...
  logic [N_SOURCE-1:0] ia_d;
  always_comb ia_d = (ia | (set & ~ia)) & (~(ia & complete_i & ~ip_o));
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ia <= '0;
    end else begin
