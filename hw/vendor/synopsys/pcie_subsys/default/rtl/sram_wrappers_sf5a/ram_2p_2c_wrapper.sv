...
// -------------------------------------------------------------------------
// -------------------------------------------------------------------------
// --- Module Description: 2-Port 2-Clock RAM wrapper for sf5a memories.
// -----------------------------------------------------------------------------

module ram_2p_2c_wrapper #(
  // ----------------------------------------------------------------------------
  // Parameters
  // ----------------------------------------------------------------------------
  ...
  case ({
  ...
  })
    {
      ...
      localparam int unsigned BankWidth    = 116;
...
`endif
      ...
      for (genvar bank_index = 0; unsigned'(bank_index) < NumBanks; bank_index++) begin : gen_data_banks
        ln05lpe_a00_mc_rf2rp_hsr_rvt_32x116m1b2c1r2_wrapper u_ra2_32x116 (
          // Basic pins
          // Write interface
          .WCK   (wclk_g),         // Write Clock
    ...
    end
    {
      ...
      localparam int unsigned BankDepth    = 32;
      localparam int unsigned BankWidth    = 84;
      localparam int unsigned AddrPadWidth = ($clog2(BankDepth) > $clog2(DP)) ? $clog2(BankDepth) - $clog2(DP) : 0;
      localparam int unsigned NumBanks     = (WD + BankWidth - 1) / BankWidth;
`ifdef DEBUG_INFO
      $info(
...
`endif
      ...
      for (genvar bank_index = 0; unsigned'(bank_index) < NumBanks; bank_index++) begin : gen_data_banks
        ln05lpe_a00_mc_rf2rp_hsr_rvt_32x84m1b2c1r2_wrapper u_rf2_32x84 (
          // Basic pins
          // Write interface
          .WCK   (wclk_g),         // Write Clock
          .WEN   (~(ena & wea)),   // Write enable, active low
    ...
    end
    {
      ...
      localparam int unsigned BankDepth    = 32;
      localparam int unsigned BankWidth    = 160;
      localparam int unsigned AddrPadWidth = ($clog2(BankDepth) > $clog2(DP)) ? $clog2(BankDepth) - $clog2(DP) : 0;
      localparam int unsigned NumBanks     = (WD + BankWidth - 1) / BankWidth;
`ifdef DEBUG_INFO
      $info(
...
`endif
      ...
      for (genvar bank_index = 0; unsigned'(bank_index) < NumBanks; bank_index++) begin : gen_data_banks
        ln05lpe_a00_mc_rf2rp_hsr_rvt_32x160m1b2c1r2_wrapper u_rf2_32x160 (
          // Basic pins
          // Write interface
          .WCK   (wclk_g),         // Write Clock
          .WEN   (~(ena & wea)),   // Write enable, active low
...
`endif
      ...
      always_ff @(posedge rclk_g) begin
    ...
    end
    {
      ...
      localparam int unsigned BankWidth    = 88;
      localparam int unsigned AddrPadWidth = ($clog2(BankDepth) > $clog2(DP)) ? $clog2(BankDepth) - $clog2(DP) : 0;
      localparam int unsigned NumBanks     = (WD + BankWidth - 1) / BankWidth;
`ifdef DEBUG_INFO
      $info(
...
`endif
      ...
      for (genvar bank_index = 0; unsigned'(bank_index) < NumBanks; bank_index++) begin : gen_data_banks
        ln05lpe_a00_mc_rf2rp_hsr_rvt_64x88m1b2c1r2_wrapper u_rf2_64x88 (
          // Basic pins
          // Write interface
          .WCK   (wclk_g),         // Write Clock
          .WEN   (~(ena & wea)),   // Write enable, active low
    ...
    end
    {
      ...
      localparam int unsigned BankDepth = 260;
      localparam int unsigned BankWidth = 128;
      localparam int unsigned NumBanks  = (DP + BankDepth - 1) / BankDepth;
`ifdef DEBUG_INFO
      $info(
        "\n",
...
`endif
      ...
      for (genvar bank_index = 0; unsigned'(bank_index) < NumBanks; bank_index++) begin : gen_data_banks
        ...
        ln05lpe_a00_mc_rf2rp_hsr_rvt_260x128m1b8c1r2_wrapper u_ra2_260x128 (
          // Basic pins
          // Write interface
          .WCK   (wclk_g),         // Write Clock
          .WEN   (~(wea & ena & wbank_en)),        // Write enable, active low
    ...
    end
    {
      ...
      ln05lpe_a00_mc_rf2rp_hsr_rvt_32x120m1b2c1r2_wrapper u_ra2_32x120 (
        // Basic pins
        // Write interface
        .WCK   (wclk_g),         // Write Clock
