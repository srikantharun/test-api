...
// Revision: $Id: //dwh/DW_ocb/DW_axi/axi_dev_br/sim/testbench/sim_svte/tb_check_lp.sv#3 $ 
**
** ---------------------------------------------------------------------
**
** File     : tb_check_lp.v
...
`ifdef AXI_QOS
  `ifdef AXI_HAS_LOWPWR_HS_IF
    wire                    apb_busy_signal ;          // used for apb activity in low power mode 
    reg                    apb_busy_signal_d1,apb_busy_signal_d2,apb_busy_signal_d3,apb_busy_signal_d4,apb_busy_signal_d5 ;// delayed apb activity signal 
    wire                   apb_busy_signal_final,apb_busy_signal_final_p;
  `endif
`endif
// Coverage points
integer                mst, mstr;
...
`ifdef AXI_QOS
  `ifdef AXI_HAS_LOWPWR_HS_IF
assign apb_busy_signal = test_DW_axi.U_DW_axi.U_DW_axi_apbif.U_DW_axi_apb_regfile.apb_busy ;

always @(posedge clk or negedge resetn) begin
  if(!resetn) begin
    apb_busy_signal_d1  <= 0;
    apb_busy_signal_d2  <= 0;
    apb_busy_signal_d3  <= 0;
    apb_busy_signal_d4  <= 0;
    apb_busy_signal_d5  <= 0;
  end
  else begin
    apb_busy_signal_d1  <= apb_busy_signal;
    apb_busy_signal_d2  <= apb_busy_signal_d1;
    apb_busy_signal_d3  <= apb_busy_signal_d2;
    apb_busy_signal_d4  <= apb_busy_signal_d3;
    apb_busy_signal_d5  <= apb_busy_signal_d4;
  end
end

`ifndef DW_MODEL_MISSAMPLES
assign apb_busy_signal_final = (`AXI_NUM_SYNC_FF==2) ? apb_busy_signal_d2 : ((`AXI_NUM_SYNC_FF==3) ? apb_busy_signal_d3 : apb_busy_signal_d4);
assign apb_busy_signal_final_p = (`AXI_NUM_SYNC_FF==2) ? apb_busy_signal_d1 : ((`AXI_NUM_SYNC_FF==3) ? apb_busy_signal_d2 : apb_busy_signal_d3);
`else
assign apb_busy_signal_final = (`AXI_NUM_SYNC_FF==2) ? apb_busy_signal_d3 : ((`AXI_NUM_SYNC_FF==3) ? apb_busy_signal_d4 : apb_busy_signal_d5);
assign apb_busy_signal_final_p = (`AXI_NUM_SYNC_FF==2) ? apb_busy_signal_d2 : ((`AXI_NUM_SYNC_FF==3) ? apb_busy_signal_d3 : apb_busy_signal_d4);
`endif
...
always @(posedge clk or negedge resetn) begin
  if(!resetn) begin
     ...
     prev_nopx_count <= nopx_cnt ;// Used for error checking
  end
end
...
// //            && ((cactive_ne && (nopx_cnt > 0))
// //               || (!cactive_ne && (nopx_cnt == 0)))
// //          //  && ((!cactive_dl && (nopx_cnt > 0))
// //          //     || (cactive_dl && (nopx_cnt == 0)))
// //
// //         // Leaving from reset
// //         && leaving_reset 
// //	 
// //	 && resetn // Check after reset de-asserted
// //
// //         ) begin
