...
A description of Hjson (a variant of JSON) and the recommended style is in the copy the lowrisc reg_tool doc [Hjson Usage and Style Guide]().
...
# Register Generator Field Reference Description
...
axi_intf | optional | python Bool | instantiate an AXI interface instead of APB
...
The basic structure of a register definition file using an AXI interface on a 64b data bus:
...
```hjson
{
  ...
  axi_intf: True
  param_list : [
    {name:"AXI_AW", type:"int", default:"36"},
    {name:"AXI_IDW", type:"int", default:"6"},
...
* CSR tests exclusions:
  ...
  Detailed description of this methodology is documented in // TODO, copy the lowrisc reg_tool doc [CSR utilities]().
...
The output is two Verilog files that can be instantiated by a peripheral that follows the // TODO, copy the lowrisc reg_tool doc [Comportability Guidelines]().
...
This means a shadow register can implement any of the types described in // TODO, copy the lowrisc reg_tool doc [this section]().
...
- Error hook-ups:
  - Storage errors signaled to the hardware in `err_storage` are fatal, and should be hooked up directly into the // TODO, copy the lowrisc reg_tool doc [alert system]().
...
- Shadow resets:
  ...
  Note that a module may need to be reset during operation of the system, i.e., when the // TODO, copy the lowrisc reg_tool doc [alert system]() is running.
