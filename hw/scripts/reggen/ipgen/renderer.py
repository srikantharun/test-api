...
import reggen.gen_dv
import reggen.gen_html
        ...
        try:
            ...
            if "module_instance_name" in self.ip_config.param_values:
                hjson_path = (
                    ...
                    (str(self.ip_config.param_values["module_instance_name"]) +
                    '.hjson'))
            ...
            # TODO: This is not supported by our version of regen
            ...
            # Also generate the ral description
            dv_path = output_dir_staging / 'dv'
            dv_path.mkdir(exist_ok=True)
            reggen.gen_dv.gen_dv(obj, dv_base_names=[], outdir=str(dv_path))

            # also generate the CSRs Documentation
            html_outdir = output_dir_staging / 'doc'
            html_outfile = (html_outdir / (obj.name + "_csr_regs")).with_suffix(".md")
            print(html_outfile)
            html_outdir.mkdir(exist_ok=True)
            with html_outfile.open("w") as f:
                reggen.gen_html.gen_html(obj, outfile=f)
