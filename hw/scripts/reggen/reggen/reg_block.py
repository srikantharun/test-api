...
class RegBlock:
    def __init__(self, reg_width: int, params: ReggenParams, reg_align: int, addrcap: int):

        self._addrsep = (reg_align + 7) // 8
        self._reg_width = reg_width
        self._params = params
        self._addrcap = addrcap
    ...
    def get_addr_width(self) -> int:
        ...
        # default:
        addr_width = (self.offset - 1).bit_length()
        # use address capacity to set the minimum address capacity
        addr_width = max(addr_width, (self._addrcap-1).bit_length())
        return addr_width
