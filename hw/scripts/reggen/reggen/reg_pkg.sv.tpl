...
% endfor
  ...
  parameter logic [3:0] ${upfx}_WR_BYTES [${len(rb.flat_regs)}] = '{
  % for i, r in enumerate(rb.flat_regs):
<%
  index_str = "{}".format(i).rjust(idx_len)
  width = r.get_width()
  if not r.needs_we(): # read only register, don't allow any write
    mask = '0000'
  elif width > 24:
    mask = '1111'
  elif width > 16:
...
%>\
  ...
  // Register information to check illegal reads${for_iface}
  parameter logic ${upfx}_REG_READABLE [${len(rb.flat_regs)}] = '{
  % for i, r in enumerate(rb.flat_regs):
<%
  index_str = "{}".format(i).rjust(idx_len)
  if r.readable():
  ...
  comma = ',' if i < len(rb.flat_regs) - 1 else ' '
%>\
    1'b${readable}${comma} // index[${index_str}] ${ublock}_${r.name.upper()}
  % endfor
  };
% endif
</%def>\

`ifndef LOWRISC_${lblock.upper()}_PKG
`define LOWRISC_${lblock.upper()}_PKG
...
% if block.axi_intf:
  ...
  typedef struct packed {
    logic [${block.regwidth}-1:0] data;
    logic [((${block.regwidth}+7)/8)-1:0] strb;
    logic last;
    logic valid;
  } axi_w_ch_h2d_t;
...
% endif
  ...
  typedef struct packed {
    logic [31:0] paddr;
    logic  pwrite;
    logic [${block.regwidth}-1:0] pwdata;
    logic psel;
    logic [2:0] pprot;
    logic penable;
    logic [(${block.regwidth}/8)-1:0] pstrb;
  } apb_h2d_t;

  typedef struct packed {
    logic pslverr;
    logic [${block.regwidth}-1:0] prdata;
    logic pready;
  } apb_d2h_t;
