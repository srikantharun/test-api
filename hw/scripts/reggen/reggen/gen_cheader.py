...
def gen_cdefines(block: IpBlock,
        ...
        for x in rb.entries:
            ...
            if isinstance(x, MultiRegister):
                gen_cdefine_multireg(outstr, x, block.name, block.regwidth, rnames,
                                     existing_defines)
                for subreg in x.regs:
                    register_list.append(subreg)
                continue
    ...
    genout(outfile, '#include <std/std_basetype.h>\n\n')
    ...
    prev_offset = 0
    reserved_cnt = 1
    genout(outfile, 'typedef struct {\n')
    for reg in register_list:
      prev_offset = offset
      offset = reg.offset
      reg_width_in_hex = int(current_block.regwidth / 8)
      if (offset - prev_offset) > reg_width_in_hex:
        num_padding = (offset - prev_offset) // reg_width_in_hex
        if prev_offset > 0: # Count out previous register from padding
            num_padding -= 1
        if num_padding == 1:
          reg_name_with_comment = f'reserved{reserved_cnt}; // 0x{prev_offset+4:02x}'
        else:
          if prev_offset == 0 and offset > reg_width_in_hex: # if first register is not at offset 0
            reg_name_with_comment = f'reserved{reserved_cnt}[{num_padding}]; // 0x{prev_offset:02x} .. 0x{offset-reg_width_in_hex:02x}'
          else:
            reg_name_with_comment = f'reserved{reserved_cnt}[{num_padding}]; // 0x{prev_offset+reg_width_in_hex:02x} .. 0x{offset-reg_width_in_hex:02x}'
        genout(outfile, f'  reg{current_block.regwidth}_t {reg_name_with_comment}\n')
        reserved_cnt += 1
      genout(outfile, f'  reg{current_block.regwidth}_t {reg.name.lower()}; // 0x{reg.offset:02x}\n')
    genout(outfile, '}' + f' Hal{current_block.name.title()}Reg;\n\n')
