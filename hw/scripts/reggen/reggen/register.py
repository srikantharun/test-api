...
READONLY = ["RO", "RC", "RS", "R1C"]
WRITEONLY = ["WO", "WOC", "WOS", "WO1"]
READWRITE = ["RW", "WRC", "WRS", "WC", "WS", "WSRC", "WCRS", "W1C", "W1S", "W1T", "W0C", "W0S", "W0T", "W1SRC", "W1CRS", "W0SRC", "W0CRS", "W1", "RW1S", "RW1C", "RW0C", "R0W1C"]
...
class Register(RegBase):
    ...
    def __init__(self,
        ...
        self.swaccessType = swaccessType
    ...
    def from_raw(reg_width: int,
        ...
        for field in fields:
          fieldType = "RW"
          if field.swaccess.key.upper() in READONLY:
            fieldType = "RO"
          elif field.swaccess.key.upper() in WRITEONLY:
            fieldType = "WO"
          swaccessFieldType.append(fieldType)

        swaccessType = SWAccess('{} register'.format(name),
                            rd.get('swaccess', 'none')).dv_rights()

        if "RW" in swaccessFieldType or ("RO" in swaccessFieldType and "WO" in swaccessFieldType):
          swaccessType = "RW"
        elif "RO" in swaccessFieldType:
          swaccessType = "RO"
        elif "WO" in swaccessFieldType:
          swaccessType = "WO"
        else:
          raise ValueError('Register {} has no swaccess.'.format(name))

        return Register(offset, name, desc, async_name, async_clk,
                        hwext, hwqe, hwre, regwen,
                        tags, resval, shadowed, fields,
                        update_err_alert, storage_err_alert, swaccessType)
    ...
    def readable(self) -> bool:
        '''Return true if at least one field is readable
        '''
        for fld in self.fields:
    ...
    def make_multi(self,
        ...
        return Register(offset, new_name, self.desc, self.async_name, self.async_clk,
                        ...
                        self.update_err_alert, self.storage_err_alert, self.swaccessType)
