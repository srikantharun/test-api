...
OPTIONAL_FIELDS = {
    ...
    'regalign': ['d', "alignment of registers in bits (default 32)"],
    'addrcap': ['d', "Minimum address capacity that should be taken into account for address decoding"],
    'axi_intf': ['pb', "Enable AXI interface instead of APB"],
...
class IpBlock:
    def __init__(self,
        ...
        self.regalign = regalign
        self.addrcap = addrcap
        self.axi_intf = axi_intf
    ...
    def from_raw(param_defaults: List[Tuple[str, str]],
        ...
        r_regalign = rd.get('regalign')
        if r_regalign is None:
            regalign = regwidth
        else:
            regalign = check_int(r_regalign, 'regalign field of ' + what)
            if regalign <= 0:
                raise ValueError('Invalid regalign field for {}: '
                                 '{} is not positive.'
                                 .format(what, regalign))

        r_addrcap = rd.get('addrcap')
        if r_addrcap is None:
            addrcap = 0
        else:
            addrcap = check_int(r_addrcap, 'addrcap field of ' + what)
            if addrcap <= 0:
                raise ValueError('Invalid addrcap field for {}: '
                                 '{} is not positive.'
                                 .format(what, addrcap))
        ...
        init_block = RegBlock(regwidth, params, regalign, addrcap)
        ...
        axi_intf = check_bool(rd.get('axi_intf', False), 'axi_intf field of ' + what)
        ...
        return IpBlock(name, regwidth, regalign, addrcap, axi_intf, params, reg_blocks,
