<%page args="module_dict, token_intf, clocks, resets"/>
<%
processed_interfaces = set()
def find_and_match_dir(sig_channel, sig, bus, match=True):
    for other_bus, bus_dict in token_intf.items():
    ...
    return ""
%>
%for intf, port_dict in token_intf.items():
% if port_dict['pipe']:
//-------------------------------
// TOKEN SPILL for ${intf}
//-------------------------------
## Generate the logic connection between spill registers and block
%for sig in module_dict["sig"]:
<%
sig_channel = '_' + {"ready": "rdy", "valid": "vld"}.get(sig.split('_')[-1], sig.split('_')[-1])
%>\
...
%if i == 0:
${conn_width} ${intf}_subip${sig_channel.ljust(3)};
%else:
${conn_width} ${intf}_subip_pipe_${i}${sig_channel.ljust(3)};
%endif
%endfor
%endif
%endfor
...
sig_channel = {"ready": "rdy", "valid": "vld"}.get(sig.split('_')[-1], sig.split('_')[-1])
%>\
%if '_clk' in sig:
    .${sig}(${clock_name})${last}
%elif '_rst' in sig:
