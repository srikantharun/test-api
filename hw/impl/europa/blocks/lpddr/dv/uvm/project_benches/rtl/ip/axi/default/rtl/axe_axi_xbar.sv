...
/// axi_xbar: AXI4+ATOP crossbar with an arbitrary number of slave and master ports.
/// See TODO:(@wolfgang.roenninger) for the documentation, including the definition of parameters and ports.
///
module axe_axi_xbar #(
  ...
  if ($bits(i_addr_map[0].addr_start) != AxiAddrWidth) $fatal(1, "Address map address wield does not match AxiAddrWidth. is: %0d and %0d",
      $bits(i_addr_map[0].addr_start),   AxiAddrWidth);
  if ($bits(i_addr_map[0].addr_end)   != AxiAddrWidth) $fatal(1, "Address map address wield does not match AxiAddrWidth. is: %0d and %0d",
  ...
  for (genvar sub_port_idx = 0; sub_port_idx < NumSubPorts; sub_port_idx++) begin : gen_sub_demux
    ...
    // TODO(@wolfgang.roenninger): Put these into a bind?
