...
module axe_axi_demux #(
  ...
  ///////////
    ...
    // TODO(@wolfgang.roenninger): replace with proper cc_lib arb
...
// TODO(@wolfgang.roenninger): Protocol checker

//  // Validate parameters.
//  `ifndef SYNTHESIS
//      default disable iff (!i_rst_n);
//      aw_select: assume property( @(posedge i_clk) (slv_req_i.aw_valid |->
//                                                   (i_axi_s_aw_select < NumPorts))) else
//        $fatal(1, "i_axi_s_aw_select is %d: AW has selected a slave that is not defined.\
//                   NumPorts: %d", i_axi_s_aw_select, NumPorts);
//      ar_select: assume property( @(posedge i_clk) (slv_req_i.ar_valid |->
//                                                   (i_axi_s_ar_select < NumPorts))) else
//        $fatal(1, "i_axi_s_ar_select is %d: AR has selected a slave that is not defined.\
//                   NumPorts: %d", i_axi_s_ar_select, NumPorts);
//      aw_valid_stable: assert property( @(posedge i_clk) (aw_valid && !aw_ready) |=> aw_valid) else
//        $fatal(1, "aw_valid was deasserted, when aw_ready = 0 in last cycle.");
//      ar_valid_stable: assert property( @(posedge i_clk)
//                                 (ar_valid && !ar_ready) |=> ar_valid) else
//        $fatal(1, "ar_valid was deasserted, when ar_ready = 0 in last cycle.");
//      slv_aw_chan_stable: assert property( @(posedge i_clk) (aw_valid && !aw_ready)
//                                 |=> $stable(slv_req_i.aw)) else
//        $fatal(1, "slv_aw_chan unstable with valid set.");
//      slv_aw_select_stable: assert property( @(posedge i_clk) (aw_valid && !aw_ready)
//                                 |=> $stable(i_axi_s_aw_select)) else
//        $fatal(1, "i_axi_s_aw_select unstable with valid set.");
//      slv_ar_chan_stable: assert property( @(posedge i_clk) (ar_valid && !ar_ready)
//                                 |=> $stable(slv_req_i.ar)) else
//        $fatal(1, "slv_ar_chan unstable with valid set.");
//      slv_ar_select_stable: assert property( @(posedge i_clk) (ar_valid && !ar_ready)
//                                 |=> $stable(i_axi_s_ar_select)) else
//        $fatal(1, "i_axi_s_ar_select unstable with valid set.");
//      internal_ar_select: assert property( @(posedge i_clk)
//          (ar_valid |-> i_axi_s_ar_select < NumPorts))
//        else $fatal(1, "i_axi_s_ar_select illegal while ar_valid.");
//      internal_aw_select: assert property( @(posedge i_clk)
//          (aw_valid |-> i_axi_s_aw_select < NumPorts))
//        else $fatal(1, "i_axi_s_aw_select illegal while aw_valid.");
//      w_underflow: assert property( @(posedge i_clk)
...
`ifndef SYNTHESIS
    cnt_underflow: assert property(
      @(posedge i_clk) disable iff (~i_rst_n) (pop_en[i] |=> !overflow)) else
        $fatal(1, "axi_demux_id_counters > Counter: %0d underflowed. The reason is probably a faulty AXI response.", i);
`endif
  end
endmodule
