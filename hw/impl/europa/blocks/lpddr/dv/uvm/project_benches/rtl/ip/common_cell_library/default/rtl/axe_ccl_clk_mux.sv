...
module axe_ccl_clk_mux #(
  ...
  always_comb begin
    ...
    state_d       = state_q.next();
    update_state  = 1'b0;

    unique case (state_q)
      ClkIsOff:   if (i_enable && !any_slice_active)       update_state = 1'b1;
      TurnClkOn:  if (any_slice_active)                    update_state = 1'b1;
      ClkIsOn:    if (!i_enable || (i_select != select_q)) update_state = 1'b1;
      TurnClkOff: if (!any_slice_active)                   update_state = 1'b1;
      default: begin
        state_d      = TurnClkOff;
        update_state = 1'b1;
      end
