...
module axe_axi_dw_downsizer #(
  ...
  // TODO(@wolfgang.roenninger): Use stream rr_arb_tree when available
  rr_arb_tree #(
    .NumIn    (2),
            ...
            if (inject_aw_into_ar) begin
              ...
              unique case (r_req_d.ar.burst)
                axi_pkg::BurstIncr : begin
                  ...
                  automatic addr_t conv_ratio = ((1 << r_req_d.ar.size) + AxiManPortStrbWidth - 1) / AxiManPortStrbWidth;

                  // Evaluate output burst length
                  automatic addr_t align_adj = (r_req_d.ar.addr & size_mask & ~MstPortByteMask) / AxiManPortStrbWidth;
            ...
            if ((idqueue_id == t) && idqueue_valid)
              ...
              if (!sub_r_valid_tran[t] || (sub_r_valid_tran[t] && sub_r_ready_tran[t])) begin
                ...
                if (axi_man_r_valid) begin
                  automatic addr_t mst_port_offset = AxiManPortStrbWidth == 1 ? '0 : r_req_q.ar.addr[cc_math_pkg::index_width(AxiManPortStrbWidth)-1:0];
                  automatic addr_t slv_port_offset = AxiSubPortStrbWidth == 1 ? '0 : r_req_q.ar.addr[cc_math_pkg::index_width(AxiSubPortStrbWidth)-1:0];
      ...
      W_PASSTHROUGH, W_INCR_DOWNSIZE, W_SPLIT_INCR_DOWNSIZE: begin
        ...
        if (!w_req_q.aw_valid)
          if (i_axi_s_w_valid) begin
            automatic addr_t mst_port_offset = AxiManPortStrbWidth == 1 ? '0 : w_req_q.aw.addr[cc_math_pkg::index_width(AxiManPortStrbWidth)-1:0];
            automatic addr_t slv_port_offset = AxiSubPortStrbWidth == 1 ? '0 : w_req_q.aw.addr[cc_math_pkg::index_width(AxiSubPortStrbWidth)-1:0];
            ...
            for (int b = 0; b < AxiSubPortStrbWidth; b++)
              if ((b >= slv_port_offset) &&
                  ...
                  (b + mst_port_offset - slv_port_offset < AxiManPortStrbWidth)) begin
                w_data[b + mst_port_offset - slv_port_offset]         = i_axi_s_w.data[8*b +: 8];
                axi_man_w.strb[b + mst_port_offset - slv_port_offset] = i_axi_s_w.strb[b]       ;
              end
      ...
      if (!forward_b_beat_full) begin
        ...
        if (i_axi_s_aw_valid && o_axi_s_aw_ready) begin
          ...
          unique case (i_axi_s_aw.burst)
            axi_pkg::BurstIncr: begin
              // Evaluate downsize ratio
              automatic addr_t size_mask  = (1 << i_axi_s_aw.size) - 1;
              automatic addr_t conv_ratio = ((1 << i_axi_s_aw.size) + AxiManPortStrbWidth - 1) / AxiManPortStrbWidth;

              // Evaluate output burst length
              automatic addr_t align_adj = (i_axi_s_aw.addr & size_mask & ~MstPortByteMask) / AxiManPortStrbWidth;
              w_req_d.burst_len          = (i_axi_s_aw.len + 1) * conv_ratio - align_adj - 1;
