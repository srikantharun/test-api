...
module axe_axi_riscv_amos #(
  ...
  function automatic int unsigned get_atop_offset;
    ...
    $fatal(1, "AW does not contain atop!");
  endfunction
        ...
        if (adapter_ready) begin
          if (atop_valid_d == LOAD || atop_valid_d == STORE) begin
            ...
            if (b_free) begin
              ...
              // o_axi_s_b.user  = axi_ax_q.user; TODO: What to do with the user as it is optional?
        ...
        if (adapter_ready) begin
          ...
          if (atop_valid_d == LOAD || atop_valid_d == STORE) begin
            ...
            if (r_free) begin
              ...
              // o_axi_s_r.user  = axi_axi_q.user; // TODO, what to do with user signal as it is optional?
      ...
      WAIT_CHANNEL_R, SEND_R: begin
        ...
        if ((r_free && (b_resp_valid || b_state_q != WAIT_COMPLETE_B)) || (r_state_q == SEND_R)) begin
          ...
          if (atop_valid_q == INVALID && atop_r_resp_q) begin
            o_axi_s_r.data = '0;
            o_axi_s_r.resp = axi_pkg::RespSlvErr;
            // o_axi_s_r.user = axi_ax_q.user; // TODO, what to do with user signal as it is optional?
          end
          if (i_axi_s_r_ready) begin
            r_state_d = FEEDTHROUGH_R;
          end else begin
