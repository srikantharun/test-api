...
// -------------------------------------------------------------------------
// -- Revision: $Id: //dwh/ddr_iip/umctl5/DWC_ddrctl_lpddr54_MAIN_BR/DWC_ddr_umctl5/src/fifo/ingot_sync_fifo_flopout_rst.sv#2 $
// -------------------------------------------------------------------------
// Description:
//
// This a generic synchronous FIFO design for power-of-2
// FIFO depths.  All outputs are flopped out.  Max of 3 gate
// delays from any input to a flop.  Latencies are the same
// as for the non-flop-out version of this FIFO, but this
// one is slightly larger in area, due to some extra flops.
//
// All FIFO contents reset to 'b0.
// ----------------------------------------------------------------------------
`include "DWC_ddrctl_all_defs.svh"
module ingot_sync_fifo_flopout_rst (
                clk,
                rstb,
                clk_gate_en,
                put,
...
input                   clk_gate_en;            // clock gate enable
...
//ccx_cond:wait_enc_start_fifo; ;01+11;the FIFO is used to store data during tweak generation, so that it shouldn't reach almost_full.
...
always @(posedge clk or negedge rstb)
    ...
    begin
        ...
        dout    <= (put & (~full) & (empty | (almost_empty & get))) ? din : mem[ram_rptr_nxt];

    end

endmodule // ingot_sync_fifo_flopout_rst
