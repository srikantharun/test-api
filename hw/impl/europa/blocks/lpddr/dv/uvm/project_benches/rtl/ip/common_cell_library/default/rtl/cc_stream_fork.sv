...
module cc_stream_fork #(
  ...
  state_e state_q[NumStreams];
  state_e state_d[NumStreams];
  ...
  for (genvar stream_idx = 0; unsigned'(stream_idx) < NumStreams; stream_idx++) begin : gen_oup_state
    ...
    always_comb o_valid[stream_idx] = (state_q[stream_idx] == Ready)
                                    &  i_select[stream_idx]
                                    &  i_valid;

    always_comb oup_transferred[stream_idx] = (state_q[stream_idx] == Wait)
                                            | ~i_select[stream_idx]
                                            | oup_transaction[stream_idx];

    always_comb begin : proc_state_transitions
      state_d[stream_idx] = state_q[stream_idx];

      unique case (state_q[stream_idx])
        Ready: if (!inp_transaction && oup_transaction[stream_idx]) state_d[stream_idx] = Wait;
        Wait:  if ( inp_transaction)                                state_d[stream_idx] = Ready;
        default:                                                    state_d[stream_idx] = Ready;
      endcase
    end

    // DFFRCL: D-Flip-Flop, asynchronous reset, synchronous clear
    always_ff @(posedge i_clk or negedge i_rst_n) begin
      if (!i_rst_n)     state_q[stream_idx] <= Ready;
      else if (i_flush) state_q[stream_idx] <= Ready;
      else              state_q[stream_idx] <= state_d[stream_idx];
    end
  end
endmodule
