//Revision: $Id: //dwh/ddr_iip/umctl5/DWC_ddrctl_lpddr54_MAIN_BR/DWC_ddr_umctl5/src/xpi/sync_fifo_checker.sv#1 $
// ------------------------------------------------------------------------------
// 
...
module sync_fifo_checker (
  ...
  property e_wr_when_fifo_full;
    @(posedge clk) disable iff (~rst_n | disable_sva_fifo_checker_wr) (ff) |-> (!wr | rd);
  endproperty  

  property e_rd_when_fifo_empty;
    @(posedge clk) disable iff (~rst_n | disable_sva_fifo_checker_rd) (fe) |-> (wr | !rd);
  endproperty 
...
//    a_rd_when_fifo_empty: assert property (e_rd_when_fifo_empty)  else ddr_tb.assertion_misc.assertion_err_msg;
    a_wr_when_fifo_full : assert property (e_wr_when_fifo_full)   else $display("-> %0t ERROR: Writing when sync fifo full", $realtime);
    a_rd_when_fifo_empty: assert property (e_rd_when_fifo_empty)  else $display("-> %0t ERROR: Reading when sync fifo empty", $realtime);
