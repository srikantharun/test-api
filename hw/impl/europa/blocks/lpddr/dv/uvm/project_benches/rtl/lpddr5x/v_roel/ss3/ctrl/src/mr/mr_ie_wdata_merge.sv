//Revision: $Id: //dwh/ddr_iip/umctl5/DWC_ddrctl_lpddr54_MAIN_BR/DWC_ddr_umctl5/src/mr/mr_ie_wdata_merge.sv#1 $
// ------------------------------------------------------------------------------
// 
...
(
  ...
  ,input  [CORE_DATA_WIDTH/8-1:0]      datamask // write data read from write data RAM
   // Output
  ,output [CORE_DATA_WIDTH-1:0]        merged_rdata
  ,output [CORE_DATA_WIDTH/8-1:0]      merged_rdatamask
);

   // Localparam
   localparam DATA_BEAT_NUM            = `MEMC_DFI_DATA_WIDTH/`MEMC_DRAM_DATA_WIDTH;
   localparam DRAM_MASK_WIDTH          = `MEMC_DRAM_DATA_WIDTH/8;
   ...
   localparam HALF_DRAM_MASK_WIDTH     = DRAM_MASK_WIDTH/2;
   localparam QUARTER_DRAM_MASK_WIDTH  = DRAM_MASK_WIDTH/4;

   // Wire
   wire [CORE_DATA_WIDTH-1:0]          full_bus_rdata;
   wire [(CORE_DATA_WIDTH/2)-1:0]      half_bus_rdata;
   wire [CORE_DATA_WIDTH/8-1:0]        full_bus_rdatamask;
   wire [(CORE_DATA_WIDTH/8)/2-1:0]    half_bus_rdatamask;
   

   assign full_bus_rdata = data;
   assign full_bus_rdatamask = datamask;
...
//SMD: Self determined expression '(i * HALF_DRAM_DATA_WIDTH)' found in module 'mr_ie_wdata_merge'
//SJ: This coding style is acceptable and there is no plan to change it.
   generate
      for (i=0; i<DATA_BEAT_NUM; i=i+1) begin : MERGED_DATA_GEN
         ...
         assign half_bus_rdatamask[i*HALF_DRAM_MASK_WIDTH+:HALF_DRAM_MASK_WIDTH] = datamask[i*DRAM_MASK_WIDTH+:HALF_DRAM_MASK_WIDTH];
      end
   endgenerate
//spyglass enable_block SelfDeterminedExpr-ML
   ...
   assign merged_rdatamask = 
                            (data_bus_width == 2'b01) ? {{($bits(merged_rdatamask) - $bits(half_bus_rdatamask)){1'b0}},half_bus_rdatamask} : //HBW
                                                        full_bus_rdatamask; //FBW
