...
- Minor update to RTL in wrapper -> individual std-cell clock buffer added for each original snps subsys clock input port. This allows to defined generated clocks on these outputs and have matching clocks to the original snps clocks -> less constraints changes required
- DFT updates
    - Intercept of RTL changes
- Constraint update (see CONSTRAINTS section below)
- Added file with macro defines that should be used during elaboration
