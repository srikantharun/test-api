...
#
# Description: PT script to process DFT constraints for pve.
...
read_verilog /data/europa/pd/pve_p/block_build/dev_1701/build/fc/compile/results/pve_p.insert_dft.v.gz
...
if { $CHECK_MANUAL_CHANGES == 0 } {
    ...
    set_clock_groups -asynchronous -name clk_1 -group [get_clocks $i_clk] -group [get_clocks {ijtag_tck}] -group [get_clocks {ssn_clk}]
    set_clock_groups -physically_exclusive -group [get_clocks {bisr_clk}]

    #reset signals are static in scan mode
    set_false_path -from [get_ports i_ao_rst_n]
    set_false_path -from [get_ports i_global_rst_n]
    

    source /data/releases/europa/dft/bronze/pve/Latest/tsdb/logic_test/tsdb_outdir/dft_inserted_designs/pve_p_rtl2.dft_inserted_design/pve_p.hierfix.sdc
