...
if { $USE_MBIST_TSDB == 0 } {
  add_clock 0 i_emmc_clk  -period 5ns -pulse_always
  add_clock 0 i_periph_clk -period 10ns -pulse_always 
...
if { $USE_PREPROCESSING } {
  set_context dft -rtl -design_id rtl_fix_edt
  set_system_mode insertion
  delete_connections u_soc_periph/u_emmc/u_cdns_sdhc_top/scan_en
  delete_connections u_soc_periph/u_emmc/u_cdns_sdhc_top/scanmode
  create_connections scan_en u_soc_periph/u_emmc/u_cdns_sdhc_top/scan_en -net_name scan_en
  create_connections test_mode u_soc_periph/u_emmc/u_cdns_sdhc_top/scanmode -net_name scan_mode

  #delete_ports {scan_in scan_out}
  #delete_connections {scan_in scan_out}
...
#set_attribute_value cdn_ip6185_1990_dll_phy_hic_dll_nand2 -name exclude_from_synthesis
set_system_mode analysis

set dftSpec [create_dft_specification -sri_sib_list {edt sri_tdr_tp} -replace ]
...
# control atspeed_mode signal with TDR, atspeed signal is gating test points inside CDN IP
add_config_element $dftSpec/IjtagNetwork/HostScanInterface(sri)/Sib(sri_tdr_tp)/Tdr(sri_ctrl_tp)

add_config_element $dftSpec/IjtagNetwork/HostScanInterface(sri)/Sib(sri_tdr_tp)/Tdr(sri_ctrl_tp)/DataOutPorts/connection(0) -value u_soc_periph/u_emmc/u_cdns_sdhc_top/scan_atspeed_tmode

set_config_value $dftSpec/IjtagNetwork/HostScanInterface(sri)/Sib(sri_tdr_tp)/Tdr(sri_ctrl_tp)/reset_value "2'b00"
#set_config_value $dftSpec/IjtagNetwork/HostScanInterface(sri)/Sib(tp)/Sib(sri_ctrl_tp)/Tdr(sri_ctrl_tp)/DataOutPorts/count 1
...
proc process_dft_specification.post_insertion {[get_current_design] args} {
  ...
  foreach_in_collection pin [get_pins soc_periph_p_rtl2_tessent_edt_c1_int_inst/edt_scan_in] {
  ...
  foreach_in_collection pin [get_pins soc_periph_p_rtl2_tessent_edt_c1_int_inst/edt_scan_out] {
       set leaf [get_attribute_value_list $pin -name name]
       set pin_leaf [get_attribute_value_list $pin -name leaf_name]
       regexp $pattern $leaf match index
