...
#srun -c8 pt_shell -file sdc_gen_dft_disable.tcl   -output_log_file sdc_gen_dft_disable.log
# PT always prints 1 error, ignore it
#grep -q "Diagnostics summary: 1 error" sdc_gen_dft_disable.log || (echo "Error: Unexpected errors in sdc_gen_dft_disable" && exit 1)
...
#sed -i '/tessent_persistent_cell_all_test/s/set_case_analysis\ 1/set_false_path\ \-from/' ${workdir}"/"${block}"_p.dft_disable.sdc"
#sed -i '/tessent_persistent_cell_edt_bypass_buf/s/set_case_analysis\ 1/set_false_path\ \-from/' ${workdir}"/"${block}"_p.cnsShift.sdc"

#cp -r ${workdir}"/"${block}"_p.dft_disable.sdc" ../../synth-rtla/constraints/dft/
