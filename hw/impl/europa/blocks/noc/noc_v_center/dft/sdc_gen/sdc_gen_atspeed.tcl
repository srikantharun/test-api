...
if { $CHECK_MANUAL_CHANGES == 0 } {
    source /data/europa/pd/europa/constraints/dev/export/clocks/noc_v_center_p_atspeed_port_clocks.tcl
    ...
    set_clock_groups -asynchronous -name clk_1 \
     ...
     -group [get_clocks {sdma_0_aon_clk}] \
     -group [get_clocks {sdma_0_clk}] \
     -group [get_clocks {sdma_1_aon_clk}] \
     -group [get_clocks {sdma_1_clk}]

    #reset signals are static in scan mode
    set_false_path -from [get_ports i_noc_rst_n]
    set_false_path -from [get_ports i_sdma_0_aon_rst_n]
    set_false_path -from [get_ports i_sdma_0_rst_n]
    set_false_path -from [get_ports i_sdma_1_aon_rst_n]
    set_false_path -from [get_ports i_sdma_1_rst_n]
